...
\subsection{Handwritten Lexer}
  ...
  To see keyword vs identifier, you usually use a hashmap to check for keywords (e.g. \texttt{if} $\to$ \texttt{<IF>}), and if there are no matches, then you catch it with a regex. 
...
\subsection{Implementations in Languages} 

  In C, preprocessing directives are a part of lexing. For example, \texttt{\#include} literally copies and pastes the contents of the target module into the current file. With \texttt{\#IFDEF}, \texttt{\#IFNDEF}, \texttt{\#ENDIF}, we can delete tokens from our file to do \textit{conditional compilation}. 

  While the concept of ``importing'' other files into the current file is similar, the process is very different from that of---say, Python or Java.\footnote{\href{https://chatgpt.com/share/6a155f53-3404-83a7-8349-03818c8b23de}{https://chatgpt.com/share/6a155f53-3404-83a7-8349-03818c8b23de}}
  ...
  \begin{enumerate}
	...
	public static void main(String arg) {
      ...
      Basically, it just tells Java that when I write \texttt{ArrayList}, understand that I mean \texttt{java.util.ArrayList}. Now, it is clear that this is more of a name resolution process, where we are binding the \texttt{ArrayList} name to \texttt{java.util.ArrayList}. 
  \end{enumerate}

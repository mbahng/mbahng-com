\section{Homotopy}
  ...
  \begin{definition}[Homotopy]
    Let $X, Y$ be topological space and let $F_0, F_1: X \longrightarrow Y$ be continuous maps. A \textbf{homotopy} from $F_0$ to $F_1$ is a continuous map (with respect to elements $t \in [0,1]$)
    ...
    If there exists a homotopy from $F_0$ to $F_1$, then we say that $F_0$ and $F_1$ are \textbf{homotopic}, denoted
    \begin{equation}
      F_0 \simeq F_1
    \end{equation}
  \end{definition}
  ...
  \begin{definition}[Relative Homotopy]
    ...
    \begin{equation}
      H(x,t) = F_0 (x) = F_1 (x)
    \end{equation}
    for all $t \in I$ and $x \in S$, which is a subset of $X$, then the maps $F_0$ and $F_1$ are said to be \textbf{homotopic relative to $S$}. 
  \end{definition}
  ...
  \begin{definition}[Path Class]
    The equivalence class of a path $f$ is called a \textbf{path class}, denoted $[f]$. Note that in the diagram above, there is only one equivalence class of paths. 
  \end{definition}

...
\subsection{Algebra of Linear Maps}
  ...
  \begin{definition}[Operator Norm]
		Let $V, W$ be normed vector spaces. Given linear map $A \in \mathcal{L}(V, W)$, the \textbf{operator norm} is defined 
		\begin{equation}
			\|A\| \coloneqq \sup_{\|v\| = 1} \|Av\|
		\end{equation}
  \end{definition} 

	Note that the operator norm is not an instrinstic property of the linear map. It depends on the norms endowed on the domain and codomain!

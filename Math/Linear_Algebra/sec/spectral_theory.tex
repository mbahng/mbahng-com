...
\subsection{Real and Complex Jordan Canonical Forms}
	...
	\begin{definition}[Jordan Block]
		A \textbf{complex Jordan block} is a matrix $A \in \mathbb{C}^{d \times d}$ of the form 
		\begin{equation}
			A = \begin{pmatrix}
				\lambda & &  &  & & \\
				& \ddots & & & & \\
			...
			\end{pmatrix}
		\end{equation}
		with $\lambda$'s in the main diagonal and $0$'s or $1$'s in the \textit{superdiagonal}. 
		\begin{enumerate}
	...
	\begin{definition}[Jordan Canonical Form over $\mathbb{C}$]
		...
		\begin{equation}
			J = \begin{pmatrix} A_1 & & & \\  & A_2 & & \\  & &\ddots& \\  & & & A_k \end{pmatrix}
		\end{equation}
		where each block $A_i \in \mathbb{R}^{d_i \times d_i}$ is a complex Jordan block. 
	\end{definition}
 ...
 \end{algo}
  ...
  \end{example}
	...
	\begin{example}[]
	  Another example with complex diagonalization. 
	\end{example}
	...
	\begin{definition}[Jordan Canonical Form over $\mathbb{R}$]
	...
	\begin{algo}[Eigendecomposition of Matrix]
		TBD
		Finally, the change of basis is represented through the matrix multiplication. 
		\begin{equation}
			J = P^{-1} A P, \quad P = \begin{pmatrix} |&|&|&| \\ f_1&f_2 & \ldots&f_n \\ |&|&|&|
			\end{pmatrix}
		\end{equation}
		where $f_i$ is the genuine/generalized eigenvectors corresponding to the transformation represented in the $i$th column of $J$. 
  \end{algo}
  ...
  \end{example}
	...
	\begin{example}[Rotation Around Vector in $\mathbb{R}^3$]
    \begin{figure}[H]
      \centering 
      \begin{tikzpicture}
  ...
  \end{example}
    ...
    A matrix is diagonalizable if and only if its algebraic multiplicities is equal to its geometric multiplicities. 
  \end{theorem}
	\begin{proof}
	...
	\begin{theorem}[Similarity Conditions]
		$A, B \in \mathbb{R}^{n \times n}$ are similar if and only if their eigendecompositions are the same. 
  \end{theorem}
  \begin{proof}

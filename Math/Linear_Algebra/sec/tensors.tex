\section{Tensors}
  ...
  There are multiple ways to construct tensor product spaces, but intuitively, the best explanation of it was from Aspinwall: a glorified comma. Note that all the constructions are equivalent and will lead to the exact same properties of tensors. The first method defines tensors outright as multilinear maps, without the need for a basis. 

\subsection{Tensor Product of Two Spaces}

	\begin{definition}[Tensor Product of Two Vector Spaces] 
		Let $V, W$ be two vector spaces over the same field $\mathbb{F}$. Then, 
  ...
  \begin{theorem}[Universal Property of Tensor Products]
    Let $S: V \times W \to V \otimes W$ be the canonical map $S(v, w) = v \otimes w$. For every \textbf{bilinear} map $\varphi: V \times W \longrightarrow \mathbb{F}$, there exists a unique \textbf{linear} map $\psi: V \otimes W \longrightarrow \mathbb{F}$ such that 
    \[\varphi (x, y) = \psi (x \otimes y) \; \forall x \in V, y \in W\]
  \end{theorem}
  \begin{proof}
    Since $\{e_i \otimes f_j\}$ is a basis for $V \otimes W$, any linear map $\psi$ is fully determined by its action on these basis vectors. We define $\psi$ by:
    \[\psi (e_i \otimes f_j) = \varphi (e_i, f_j)\]
    Extending this linearly to the whole space $V \otimes W$ gives the unique map required.
  \end{proof}

  This property can be visualized by the following commutative diagram. The map $S$ is not an isomorphism, but it is the universal bilinear map.\footnote{Thanks to Thomas Kidane for pointing out a previous error.}
  \[\begin{tikzcd}
      V \times W \arrow[r, "\varphi"] \arrow[d, "S"'] & \mathbb{F} \\
      V \otimes W \arrow[ru, "\psi"']
  \end{tikzcd}\]
  That is, $\psi \circ S = \varphi$. This establishes a canonical isomorphism between the space of bilinear forms on $V \times W$ and the dual space of the tensor product:
  \[ \text{Bilinear}(V \times W, \mathbb{F}) \cong (V \otimes W)^* \]
  ...
  \begin{equation}
    V^* \otimes W \simeq \text{Hom}(V, W) \label{linmaps_are_tensors}
  \end{equation}
  That is, an element $T = \sum \alpha_i \otimes w_i \in V^* \otimes W$ can be interpreted as a linear map $T: V \to W$. Explicitly, for any $v \in V$:
  ...
  \end{align*}
  The coefficients $A_{ji}$ correspond exactly to the matrix entries of the linear map. This realization of a tensor product between a covector and a vector is often called the \textbf{outer product}.

  \begin{definition}
    Given vector spaces $U, V$ with coordinate vectors $u \in \mathbb{F}^m$ and $v \in \mathbb{F}^n$, the \textbf{outer product} is defined as the matrix multiplication $u v^T$:
    \[u \otimes_{\text{outer}} v \equiv u v^T = 
...
\subsection{Higher Order Tensor Product Spaces}
  ...
  which allows us to write tensor products of any finite number of vector spaces $V_1, V_2, \cdots, V_n$ without parantheses. By induction, we can keep constructing higher order tensor products as such 
  \[V_1 \otimes V_2 \rightarrow (V_1 \otimes V_2) \otimes V_3 \rightarrow \big((V_1 \otimes V_2) \otimes V_3 \big) \otimes V_4 \rightarrow \cdots\]

  to get the tensor product space
  \[\bigotimes_{i=1}^{n} V_{i} \equiv V_1 \otimes V_2 \otimes \cdots \otimes V_n\]

  with tensors in the form 
  \[ \bigotimes_{i=1}^{n} v_{i} \equiv v_{1} \otimes v_{2} \otimes v_{3} \otimes \cdots \otimes v_{n}; \; v_{i} \in V_{i}\]

  defined as the following multilinear map 
  \[ \bigotimes_{i=1}^{n} v_{i}: \prod_{i=1}^{n} V_{i}^{*} \longrightarrow \mathbb{F}, \;\;\; \bigg( \bigotimes_{i=1}^{n} v_{i} \bigg) \big( l_{1}, l_{2}, \cdots, l_{n} \big) \equiv \prod_{i=1}^{n} v_{i}(l_{i}), \; l_i \in V_i^* \]
  ...
  Similarly to the section about the tensor product of two spaces, we can "partially" fill in the inputs of a general tensor $v_1 \otimes v_2 \otimes \cdots \otimes v_n$ to interpret them as multilinear operators that can take in $k$ vectors and output $n-k$ vectors. That is, tensors (written as $\tau$ below) are multilinar maps from a cartesian product of vector spaces to a tensor product of vector spaces. 
  ...
  \text{Hom}\Big( \prod_{i=3}^n V_i^*, V_1 \otimes V_2 \Big) \simeq \cdots \]
  Furthermore, we can generalize the universal property of two tensors to the following theorem, which is also called the \textbf{fundamental principle of tensor algebra}. 

  \begin{theorem}[Universal Property of $n$-tensors]
    Given a \textbf{multilinear} mapping $\varphi: V_1 \times \dots \times V_n \longrightarrow \mathbb{F}$, there exists a unique \textbf{linear} map $\psi: V_1 \otimes \dots \otimes V_n \longrightarrow \mathbb{F}$ such that
    \[ \varphi(v_1, \dots, v_n) = \psi(v_1 \otimes \dots \otimes v_n) \]
    This correspondence establishes the canonical isomorphism:
    \[ \bigg( \bigotimes_{i=1}^n V_i \bigg)^* \simeq \mathcal{L}(V_1, \dots, V_n; \mathbb{F}) \]
    where $\mathcal{L}(V_1, \dots, V_n; \mathbb{F})$ denotes the vector space of all $n$-linear forms on the Cartesian product $V_1 \times \dots \times V_n$.
  \end{theorem}
...
\subsection{Contractions, Tensor Algebras}
  ...
  \begin{align*}
    T_{\mu_{1}, \cdots, \mu_{n}} \bigotimes_{i=1}^{n} e^{\mu_{i}} & \equiv T_{\mu_{1}, \cdots, \mu_{n}} e^{\mu_{1}} \otimes e^{\mu_{2}} \otimes \cdots \otimes e^{\mu_{n}} \\
       & \equiv \sum_{\mu_{1}, \cdots, \mu_{n}} T_{\mu_{1}, \cdots, \mu_{n}} e^{\mu_{1}} \otimes e^{\mu_{2}} \otimes \cdots \otimes e^{\mu_{n}} \\
       & \equiv \sum_{\mu_{1}, \cdots, \mu_{n}} T_{\mu_{1}, \cdots, \mu_{n}} \bigotimes_{i=1} e^{\mu_{i}}
  \end{align*}
  Since the coefficients of the shorthand tensor notation implies the tensors themselves, we can simply write
  \[ T_{\mu_{1}, \cdots, \mu_{n}} \equiv T_{\mu_{1}, \cdots, \mu_{n}} \bigotimes_{i=1}^{n} e^{\mu_{i}} \]
  ...
  \begin{theorem}
    ...
    \cdots \\ x \\ \cdots 
    \end{pmatrix} = \begin{pmatrix}
     A_1 \cdot x \\ A_2 \cdot x \\ \cdots \\ A_n \cdot x
    \end{pmatrix}\]
  \end{theorem}
  ...
  \begin{definition}
    ...
    \begin{align*}
      T(V) \equiv \bigoplus_{n = 0}^{\infty} V^{\otimes n} & = V^{\otimes 0} \oplus V^{\otimes 1} \oplus V^{\otimes 2} \oplus V^{\otimes 3} \oplus \cdots \\
...
\subsection{Exterior Algebras and Symmetric Algebras}
  ...
  Generally, if $\sigma$ is a permutation of the ordered list $(1, 2, \cdots, n)$, and $x_1, x_2, \cdots, x_n \in V$, then 
  \[x_{\sigma(1)} \wedge x_{\sigma(2)} \wedge \cdots \wedge_{\sigma(n)} = sgn(\sigma) \; x_1 \wedge x_2 \wedge \cdots \wedge x_n\]
  ...
  \begin{align*}
    \Lambda(V) \equiv \bigoplus_{k=0}^n \Lambda^k V \equiv \Lambda^0 V \oplus \Lambda^1 V \oplus \Lambda^2 V \oplus \cdots \oplus \Lambda^n V 
  \end{align*}
  ...
  \begin{definition}
    ...
    This vector is the \textit{determinant}. Note that this construction of the determinant is consistent with our previous construction of the determinant of a matrix since $e_1 \wedge \cdots \wedge e_n$ is indeed multilinear and antisymmetric. In its purest sense, 
    ...
    Note that any tensor in $\Lambda^n V$ satisfies multilinearity and antisymmetricity, but only the basis vector $e_1 \wedge \cdots \wedge e_n$ satisfies the normalizing condition
    \[\det{I} = 1\]
    Since, given that the dual basis of $V^*$ is $\{f_j\}$
    \[(e_1 \wedge \cdots \wedge e_n) (f_1, f_2, \cdots, f_n) = \prod_{i=1}^n e_i (f_i) = \prod_{i=1}^n \delta_i^i = 1\]
  \end{definition}
...
\subsection{Determinants and Trace}
  ...
  \begin{algo}[Dodgson Condensation]
    We will introduce another, called \textbf{Dodgson condensation}. The algorithm can be described in the following steps.

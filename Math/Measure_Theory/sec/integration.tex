...
\subsection{Lebesgue Integral for Signed Functions} 
  ...
  \begin{example}[Signed Measure from Function]
...
\subsection{Uniformly Integrable Families} 
  ...
  \begin{corollary}
		A corollary of \hyperref[thm:riesz]{this theorem}. Let $f_n \geq 0$, integrable on $E$. Then, 
    \begin{equation}
...
\subsection{Exercises} 
  ...
  \end{solution}
	...
	\begin{exercise}[Folland 2.1]
		Let $f : X \to \overline{\mathbb{R}}$ and $Y = f^{-1}(\mathbb{R})$. Then $f$ is measurable iff $f^{-1}(\{-\infty\}) \in \mathcal{M}$, $f^{-1}(\{\infty\}) \in \mathcal{M}$, and $f$ is measurable on $Y$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.2]
		Suppose $f, g : X \to \overline{\mathbb{R}}$ are measurable.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item $fg$ is measurable (where $0 \cdot (\pm\infty) = 0$).
			\item Fix $a \in \overline{\mathbb{R}}$ and define $h(x) = a$ if $f(x) = -g(x) = \pm\infty$ and $h(x) = f(x) + g(x)$ otherwise. Then $h$ is measurable.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.6]
		The supremum of an uncountable family of measurable $\overline{\mathbb{R}}$-valued functions on $X$ can fail to be measurable (unless the $\sigma$-algebra $\mathcal{M}$ is very special).
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.7]
		Suppose that for each $\alpha \in \mathbb{R}$ we are given a set $E_\alpha \in \mathcal{M}$ such that $E_\alpha \subset E_\beta$ whenever $\alpha < \beta$, $\bigcup_{\alpha \in \mathbb{R}} E_\alpha = X$, and $\bigcap_{\alpha \in \mathbb{R}} E_\alpha = \varnothing$. Then there is a measurable function $f : X \to \mathbb{R}$ such that $f(x) \le \alpha$ on $E_\alpha$ and $f(x) \ge \alpha$ on $E_\alpha^c$ for every $\alpha$. (Use Exercise 4.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.8]
	...
	\begin{exercise}[Folland 2.9]
		...
		\begin{enumerate}[label=\textbf{\alph*.}]
			...
			\item There exist a Lebesgue measurable function $F$ and a continuous function $G$ on $\mathbb{R}$ such that $F \circ G$ is not Lebesgue measurable.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.13]
		Suppose $\{f_n\} \subset L^+$, $f_n \to f$ pointwise, and $\int f = \lim \int f_n < \infty$. Then $\int_E f = \lim \int_E f_n$ for all $E \in \mathcal{M}$. However, this need not be true if $\int f = \lim \int f_n = \infty$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.14]
		If $f \in L^+$, let $\lambda(E) = \int_E f \, d\mu$ for $E \in \mathcal{M}$. Then $\lambda$ is a measure on $\mathcal{M}$, and for any $g \in L^+$, $\int g \, d\lambda = \int fg \, d\mu$. (First suppose that $g$ is simple.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.15]
	...
	\begin{exercise}[Folland 2.19]
		Suppose $\{f_n\} \subset L^1(\mu)$ and $f_n \to f$ uniformly.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item If $\mu(X) < \infty$, then $f \in L^1(\mu)$ and $\int f_n \to \int f$.
			\item If $\mu(X) = \infty$, the conclusions of (a) can fail. (Find examples on $\mathbb{R}$ with Lebesgue measure.)
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.23]
		Given a bounded function $f : [a, b] \to \mathbb{R}$, let
		\begin{align}
			H(x) &= \lim_{\delta \to 0} \sup_{|y-x| \le \delta} f(y), \\
			h(x) &= \lim_{\delta \to 0} \inf_{|y-x| \le \delta} f(y).
		\end{align}
	...
	\begin{exercise}[Folland 2.24]
		Let $(X, \mathcal{M}, \mu)$ be a measure space with $\mu(X) < \infty$, and let $(X, \overline{\mathcal{M}}, \overline{\mu})$ be its completion. Suppose $f : X \to \mathbb{R}$ is bounded. Then $f$ is $\overline{\mathcal{M}}$-measurable (and hence in $L^1(\overline{\mu})$) iff there exist sequences $\{\phi_n\}$ and $\{\psi_n\}$ of $\mathcal{M}$-measurable simple functions such that $\phi_n \le f \le \psi_n$ and $\int (\psi_n - \phi_n) \, d\mu < n^{-1}$. In this case, $\lim \int \phi_n \, d\mu = \lim \int \psi_n \, d\mu = \int f \, d\overline{\mu}$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.25]
	...
	\begin{exercise}[Folland 2.26]
		If $f \in L^1(m)$ and $F(x) = \int_{-\infty}^x f(t) \, dt$, then $F$ is continuous on $\mathbb{R}$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.27]
	...
	\begin{exercise}[Folland 2.29]
		Show that $\int_0^\infty x^n e^{-x} \, dx = n!$ by differentiating the equation $\int_0^\infty e^{-tx} \, dx = 1/t$. Similarly, show that $\int_{-\infty}^\infty x^{2n} e^{-x^2} \, dx = (2n)!\sqrt{\pi}/4^n n!$ by differentiating the equation $\int_{-\infty}^\infty e^{-tx^2} \, dx = \sqrt{\pi/t}$ (see Proposition 2.53).
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.30]
	...
	\begin{exercise}[Folland 2.31]
		...
		\begin{enumerate}[label=\textbf{\alph*.}]
			...
			\item For $a > 1$, $\int_0^\infty e^{-ax} J_0(x) \, dx = (s^2 + 1)^{-1/2}$, where $J_0(x) = \sum_0^\infty (-1)^n x^{2n} / 4^n (n!)^2$ is the Bessel function of order zero.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.35]
		$f_n \to f$ in measure iff for every $\epsilon > 0$ there exists $N \in \mathbb{N}$ such that $\mu(\{x : |f_n(x) - f(x)| \ge \epsilon\}) < \epsilon$ for $n \ge N$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.36]
	...
	\begin{exercise}[Folland 2.37]
		Suppose that $f_n$ and $f$ are measurable complex-valued functions and $\phi : \mathbb{C} \to \mathbb{C}$.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item If $\phi$ is continuous and $f_n \to f$ a.e., then $\phi \circ f_n \to \phi \circ f$ a.e.
			\item If $\phi$ is uniformly continuous and $f_n \to f$ uniformly, almost uniformly, or in measure, then $\phi \circ f_n \to \phi \circ f$ uniformly, almost uniformly, or in measure, respectively.
			\item There are counterexamples when the continuity assumptions on $\phi$ are not satisfied.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.38]
		Suppose $f_n \to f$ in measure and $g_n \to g$ in measure.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item $f_n + g_n \to f + g$ in measure.
			\item $f_n g_n \to fg$ in measure if $\mu(X) < \infty$, but not necessarily if $\mu(X) = \infty$.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.42]
		Let $\mu$ be counting measure on $\mathbb{N}$. Then $f_n \to f$ in measure iff $f_n \to f$ uniformly.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.43]
	...
	\begin{exercise}[Folland 2.44]
		(\textbf{Lusin's Theorem}) If $f : [a, b] \to \mathbb{C}$ is Lebesgue measurable and $\epsilon > 0$, there is a compact set $E \subset [a, b]$ such that $\mu(E^c) < \epsilon$ and $f|E$ is continuous. (Use Egoroff's theorem and Theorem 2.26.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.45]
		If $(X_j, \mathcal{M}_j)$ is a measurable space for $j = 1, 2, 3$, then $\bigotimes_1^3 \mathcal{M}_j = (\mathcal{M}_1 \otimes \mathcal{M}_2) \otimes \mathcal{M}_3$. Moreover, if $\mu_j$ is a $\sigma$-finite measure on $(X_j, \mathcal{M}_j)$, then $\mu_1 \times \mu_2 \times \mu_3 = (\mu_1 \times \mu_2) \times \mu_3$.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.46]
		Let $X = Y = [0, 1]$, $\mathcal{M} = \mathcal{N} = \mathcal{B}_{[0, 1]}$, $\mu = \text{Lebesgue measure}$, and $\nu = \text{counting measure}$. If $D = \{(x, x) : x \in [0, 1]\}$ is the diagonal in $X \times Y$, then $\iint \chi_D \, d\mu \, d\nu$, $\iint \chi_D \, d\nu \, d\mu$, and $\int \chi_D \, d(\mu \times \nu) = \mu \times \nu(D)$ are all unequal. (To compute $\int \chi_D \, d(\mu \times \nu) = \mu \times \nu(D)$, go back to the definition of $\mu \times \nu$.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.47]
		Let $X = Y$ be an uncountable linearly ordered set such that for each $x \in X$, $\{y \in X : y < x\}$ is countable. (Example: the set of countable ordinals.) Let $\mathcal{M} = \mathcal{N}$ be the $\sigma$-algebra of countable or co-countable sets, and let $\mu = \nu$ be defined on $\mathcal{M}$ by $\mu(A) = 0$ if $A$ is countable and $\mu(A) = 1$ if $A$ is co-countable. Let $E = \{(x, y) \in X \times X : y < x\}$. Then $E_x$ and $E^y$ are measurable for all $x, y$, and $\iint \chi_E \, d\mu \, d\nu$ and $\iint \chi_E \, d\nu \, d\mu$ exist but are not equal. (If one believes in the continuum hypothesis, one can take $X = [0, 1]$ [with a nonstandard ordering] and thus obtain a set $E \subset [0, 1]^2$ such that $E_x$ is countable and $E^y$ is co-countable [in particular, Borel] for all $x, y$, but $E$ is not Lebesgue measurable.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.48]
		Let $X = Y = \mathbb{N}$, $\mathcal{M} = \mathcal{N} = \mathcal{P}(\mathbb{N})$, $\mu = \nu = \text{counting measure}$. Define $f(m, n) = 1$ if $m = n$, $f(m, n) = -1$ if $m = n + 1$, and $f(m, n) = 0$ otherwise. Then $\int |f| \, d(\mu \times \nu) = \infty$, and $\iint f \, d\mu \, d\nu$ and $\iint f \, d\nu \, d\mu$ exist and are unequal.
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.49]
		Prove Theorem 2.39 by using Theorem 2.37 and Proposition 2.12 together with the following lemmas.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item If $E \in \mathcal{M} \times \mathcal{N}$ and $\mu \times \nu(E) = 0$, then $\nu(E_x) = \mu(E^y) = 0$ for a.e.\ $x$ and $y$.
	...
	\begin{exercise}[Folland 2.63]
		The technique used to prove Proposition 2.54 can also be used to integrate any polynomial over $S^{n-1}$. In fact, suppose $f(x) = \prod_1^n x_j^{\alpha_j}$ ($\alpha_j \in \mathbb{N} \cup \{0\}$) is a monomial. Then $\int f \, d\sigma = 0$ if any $\alpha_j$ is odd, and if all $\alpha_j$'s are even,
		\begin{equation}
			\int f \, d\sigma = \frac{2\Gamma(\beta_1)\cdots\Gamma(\beta_n)}{\Gamma(\beta_1 + \cdots + \beta_n)}, \quad\text{where } \beta_j = \frac{\alpha_j + 1}{2}.
		\end{equation}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 2.64]
		For which real values of $a$ and $b$ is $|x|^a |\log |x||^b$ integrable over $\{x \in \mathbb{R}^n : |x| < \frac{1}{2}\}$? Over $\{x \in \mathbb{R}^n : |x| > 2\}$?
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 2.65]
		Define $G : \mathbb{R}^n \to \mathbb{R}^n$ by $G(r, \phi_1, \dots, \phi_{n-2}, \theta) = (x_1, \dots, x_n)$ where
		\begin{align}
			x_1 &= r \cos \phi_1, \quad x_2 = r \sin \phi_1 \cos \phi_2, \quad x_3 = r \sin \phi_1 \sin \phi_2 \cos \phi_3, \dots, \\
			x_{n-1} &= r \sin \phi_1 \cdots \sin \phi_{n-2} \cos \theta, \quad x_n = r \sin \phi_1 \cdots \sin \phi_{n-2} \sin \theta.
		\end{align}
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item $G$ maps $\mathbb{R}^n$ onto $\mathbb{R}^n$, and $|G(r, \phi_1, \dots, \phi_{n-2}, \theta)| = |r|$.
			\item $\det D_{(r, \phi_1, \dots, \phi_{n-2}, \theta)} G = r^{n-1} \sin^{n-2} \phi_1 \sin^{n-3} \phi_2 \cdots \sin \phi_{n-2}$.
			\item Let $\Omega = (0, \infty) \times (0, \pi)^{n-2} \times (0, 2\pi)$. Then $G|\Omega$ is a diffeomorphism and $m(\mathbb{R}^n \setminus G(\Omega)) = 0$.
			...
			\begin{equation}
				d\sigma(\phi_1, \dots, \phi_{n-2}, \theta) = \sin^{n-2} \phi_1 \sin^{n-3} \phi_2 \cdots \sin \phi_{n-2} \, d\phi_1 \cdots d\phi_{n-2} \, d\theta.
			\end{equation}
		\end{enumerate}
	\end{exercise}
	\begin{solution}

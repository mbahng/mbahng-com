...
\begin{exercise}[Tao 1.2.2]
  Give an example of a sequence of uniformly bounded, Riemann integrable functions $f_n : [0,1] \to \mathbf{R}$ for $n = 1, 2, \ldots$ that converge pointwise to a bounded function $f : [0,1] \to \mathbf{R}$ that is \emph{not} Riemann integrable. What happens if we replace pointwise convergence with uniform convergence?
\end{exercise}
\begin{solution}
  The only non-Riemann integrable function that we know of is the Riemann function, so let's a try to construct such a limit with this. Enumerate the rationals $\mathbb{Q} = \{q_k\}_{k \in \mathbb{N}}$. Now, consider the function 
  \begin{equation}
    f (x) = \begin{cases} 

...
\subsection{Convergence} 
  ...
  Practically, proving convergence in measure is still pretty good since we can pass in a subsequence that converges a.e. 
...
\subsection{Exercises} 
  ...
  \begin{exercise}[Math 631 Fall 2025, Midterm Exercise 4]
    Suppose that $f_n : \mathbb{R} \mapsto \mathbb{R}$ is a collection of measurable functions, $n = 1, 2, \dots$ Define $f(x) = \inf_n f_n(x)$. Suppose that $f(x)$ is a.e. finite. Is $f(x)$ necessarily measurable?
  \end{exercise}
  \begin{solution}
    Fix any $c \in \mathbb{R}$. Since $f_n$ are measurable, the sets $E_{c,n} = \{x \mid f_n(x) < c\}$ are all measurable. But we also have that $E_c = \{x \mid f(x) < c\}$ satisfies $E_c = \cup_{n=1}^\infty E_{c,n}$. Indeed, for every $n$, $E_{c,n} \subset E_c$ by definition of the infimum. Suppose now $x \in E_c$, so $f(x) < c$. Then there must be $n$ such that $f_n(x) < c$, otherwise we again get a contradiction with properties of the infimum. But a countable union of measurable sets is measurable, so $E_c$ is measurable. It follows that $f$ is measurable.
  \end{solution}

  \begin{exercise}[Math 631 Fall 2025, Midterm Exercise 5]
    Suppose that functions $f_n$ satisfy $f_n(x) \rightarrow 0$ as $n \rightarrow \infty$ for a.e. $x \in [0, 1]$. Prove that there exist a sequence of numbers $A_n \rightarrow \infty$ such that $A_n f_n(x) \rightarrow 0$ for a.e. $x \in [0, 1]$.
  \end{exercise}
  \begin{solution}
    Denote $E_{\epsilon,N}$ the set of all $x$ such that $|f_n(x)| < \epsilon$ for all $n \ge N$. For every $\epsilon > 0$, we have that $E_{\epsilon,N}$ are ascending in $N$ and $\cup_N E_{\epsilon,N} = [0, 1] \setminus E_0$ where $m(E_0) = 0$. Take $\epsilon_k = 2^{-k}$, and choose an increasing sequence $N_k, k = 1, 2, \dots$ so that $1 - m(E_{2^{-k}, N_k}) \le 2^{-k}$. We can find such $N_k$ since by continuity of Lebesgue measure,
    \begin{equation}
      m(E_{2^{-k},n}) \rightarrow m([0, 1] \setminus E_0) = 1
    \end{equation}
    as $n \rightarrow \infty$. We can also enforce $N_{k+1} > N_k$ at each step. Set $N_0 = 0$. Now let us define $A_n = k$ if $N_k < n \le N_{k+1}$. We claim that $A_n f_n(x)$ converges to 0 a.e. Indeed, notice that the sets $F_k = [0, 1] \setminus E_{2^{-k}, N_k}$ satisfy $m(F_k) \le 2^{-k}$. Therefore, By Borel-Cantelli lemma, a.e. $x \in [0, 1]$ belongs to only finite number of $F_k$. Thus for a.e. $x$ there exists $M(x)$ such that $x \in E_{2^{-k}, N_k}$ for all $k \ge M(x)$. Given $\epsilon > 0$, find $K \ge M(x)$ such that $k2^{-k} < \epsilon$ for all $k \ge K$. Then for every $n > N_K$, find $j \ge K$ so that $N_j < n \le N_{j+1}$. We have
    \begin{align}
      |A_n f_n(x)| &= |j f_n(x)| \\
      &\le j2^{-j} < \epsilon.
    \end{align}
    The second step follows from $x \in E_{2^{-j}, N_j}$ since $j \ge K \ge M(x)$.
  \end{solution}

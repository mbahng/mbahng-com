\section{Measures}
  ...
  \end{definition} 
	...
	The size of such boxes are well known. 
...
\subsection{Jordan Measure}
  ...
  \begin{lemma}[Fundamental Properties of Elementary Sets]
		\label{thm:set_prop_elementary_sets}
  ...
  It may be hard to tell directly whether something is Jordan measurable. This is where the ``Cauchy criterion'' of Jordan measurable sets comes in, which is very similar to \hyperref[real-thm:cauchy-riemann-integrability]{this theorem on Riemann integrability}. 

  \begin{theorem}[Cauchy Criterion of Jordan Measurability]
  ...
  \begin{lemma}[Boolean Closure of Jordan Measurable Sets]
    ...
    \begin{enumerate}
      \item $E \cup F$ is Jordan-measurable. 
      \item $E \cap F$ is Jordan-measurable. 
      \item $E \setminus F$ is Jordan-measurable. 
    \end{enumerate}
  \end{lemma}
  \begin{proof}
  ...
  \end{theorem}
    ...
    The following sets are Jordan measurable. 
		\begin{enumerate}
		  \item Regions under bounded graphs. 
			\item Triangles. 
			\item All compact convex polytopes. 
			\item Open and closed balls in $\mathbb{R}^n$. 
		\end{enumerate}
  \end{example}
  ...
  Jordan measurable sets are sets that are ``almost'' elementary, but a few sets already come to mind that are not Jordan measurable. 
	...
	\begin{example}[Non-Jordan Measurable Sets]
  ...
  \begin{proof}
		...
		\begin{enumerate}
			\item $m^\ast (E) \leq m^\ast (E \cap F) + m^\ast(E \setminus F)$. By 
		  \item $m^\ast (E) \geq m^\ast (E \cap F) + m^\ast(E \setminus F)$. By definition, for any $\epsilon > 0$, there exists an elementary set $A \supset E$ s.t. 
				\begin{equation}
				  |A| < m^\ast (E) + \epsilon
				\end{equation}
				Then, $A \cap F$ and $A \setminus F$ \hyperref[thm:set_prop_elementary_sets]{are elementary}. Therefore, we can take the disjoint partiiton $A = (A \cap F) \cup (A \setminus F)$ and see that $|A| = |A \cap F| + |A \setminus F|$. By taking the infimum of both sides, we get 
				\begin{align}
					m^\ast (A) = \inf_{A \supset E} |A| 
						& = \inf_{A \supset E} \big( |A \cap F| + |A \setminus F| \big) \\ 
						& \geq \inf_{A \supset E} |A \cap F| + \inf_{A \supset E} |A \setminus F| \\ 
						& \geq \inf_{B \supset (A \cap F)} |B| + \inf_{B \supset (A \setminus F)} |B| \\ 
						& = m^\ast (A \cap F) + m^\ast (A \setminus F)
				\end{align}
		\end{enumerate}
  \end{proof}
  ...
  In some applications, we can just ignore some of these sets as ``pathological.'' But for Riemann integrability, we are integrating over Jordan measurable sets (closed and bounded intervals). It turns out that it is really important that we can \textit{at least} guarantee that countable unions and intersections of Jordan measurable sets are measurable. If we don't, then not even uniform convergence---an extremely strong property---can preserve continuity, differentiability, and integrability of a sequence of functions. 
	...
	We tinker around with the definition of the Jordan measure. Note that by definition, the Jordan outer measure can be equivalently written as 
  \begin{align}
...
\subsection{Pi-Systems and Semialgebras}

	Note that when constructing the Jordan measure, we started off with a collection of elementary sets (the intervals), which satisfied the property that the intersection of any two intervals is an interval. This motivates the following definition. 

	\begin{definition}[$\pi$-System]
		A collection of sets $\mathcal{P}$ is a \textbf{$\pi$-system} if it is closed under finite intersections. That is for $E_1, \ldots, E_n \in \mathcal{P}$, 
		\begin{equation}
			\bigcap_{i=1}^n E_i \in \mathcal{P}
		\end{equation}
	\end{definition}

	\begin{example}[Boxes are $\pi$-Systems in $\mathbb{R}^n$]
		The set of all boxes is a $\pi$-system in $\mathbb{R}^n$. 
	\end{example}

	A slightly stronger formulation is the following. 

  \begin{definition}[Semialgebra]
    Given a set $X$, a collection $\mathcal{S}$ of subset of $X$ is said to be a \textbf{semialgebra} that satisfies the following. 
    \begin{enumerate}
			\item \textit{Contains Empty Set}. $\emptyset \in \mathcal{S}$. 
			\item \textit{Closure under Finite Intersections}. If $E_1, \ldots, E_n \in \mathcal{S}$, then 
				\begin{equation}
					\bigcap_{i=1}^n E_i \in \mathcal{S}
				\end{equation}

			\item \textit{Disjoint Union of Complements}. If $E \in \mathcal{S}$, then $E^c$ is a finite disjoint union of sets in $\mathcal{S}$. That is, there exists $E_1, \ldots, E_n \in \mathcal{S}$ such that 
				\begin{equation}
					E^c = \bigsqcup_{i=1}^n E_i
				\end{equation}
    \end{enumerate}
  \end{definition}
...
\subsection{Algebras and Sigma Algebras} 
  ...
  Now, given any set $X$, we can construct its power set $2^X$. But we can't naively just give a measure to every $A \in 2^X$, since for certain spaces, this causes nasty contradictions shown through the Banach-Tarski Paradox.\footnote{Given any two bounded subsets $A$ and $B$ of $\mathbb{R}^n$ where $n \geq 3$, both of which have a nonempty interior, there are partitions of $A$ and $B$ into a finite number of disjoint subsets, $A = A_1 \cup \ldots \cup A_k$, $B = B_1 \cup \ldots \cup B_k$, such that $A_i$ and $B_i$ are congruent for every $i \in [k]$.} A nice set of subsets of $X$ to work with is the $\sigma$-algebra of $X$. 
	...
	\begin{definition}[Algebra]
		An \textbf{algebra} of sets $\mathcal{A}$ is a nonempty collection of sets that is 
		\begin{enumerate}
		  \item closed under finite unions. 
  ...
  Now, we show that we can extend this semialgebra to an algebra. 
	...
	\begin{lemma}[Semialgebra to Algebra]
  ...
  \end{proof}
	...
	\begin{definition}[$\sigma$-Algebra] 
		A \textbf{$\sigma$-algebra} of sets $\mathcal{M}$ is a nonempty collection of sets such that
		\begin{enumerate}
			\item It is closed under countable unions. 
			\item It is closed under complements. 
		\end{enumerate} 
	\end{definition}

	This is the collection that we want. 

	\begin{lemma}[Algebra to $\sigma$-Algebra]
	  
	\end{lemma}

	\begin{definition}[Measure Space]
		Given some set $X$ and a $\sigma$-algebra $M \subset 2^X$, the tuple $(X, \mathcal{M})$ is called a \textbf{measure space}. 
	\end{definition}
  ...
  \begin{definition}[$\sigma$-Algebra Generated by a Set]
    Given a collection of sets $\mathcal{S}$, the $\sigma$-algebra \textbf{generated} by $\mathcal{S}$ is the unique smallest $\sigma$-algebra containing $\mathcal{S}$, denoted $\sigma(\mathcal{S})$. 
  \end{definition} 
	\begin{proof}
    It suffices to prove that this exists and is unique. Let us denote $\mathcal{M}$ as the set of all possible $\sigma$-algebras $\mathcal{B}$ of $X$. $\mathcal{M}$ is nonempty since it contains $2^X$. Then, the intersection 
    \begin{equation}
  ...
  \end{enumerate} 
	...
	We will see more examples of Borel $\sigma$-algebras when looking at measure on Euclidean space. 
...
\subsection{Measures} 
  ...
  \end{definition}
	...
	Let's introduce some classifications of measures. 

	\begin{definition}[Finite Measure]
		Let $(X, \mathcal{M}, \mu)$ be a measure space. $\mu$ is \textbf{finite} if $\mu(X) < +\infty$, and a set $E \in \mathcal{M}$ is said to be of \textbf{finite measure} if $\mu(E) < +\infty$. 
	\end{definition}

	\begin{definition}[$\sigma$-Finite Measure]
		Let $(X, \mathcal{M}, \mu)$ be a measure space. The measure $\mu$ is \textbf{$\sigma$-finite} if $X$ is the union of a countable collection of measurable sets, each of which has finite measure. $E \in \mathcal{M}$ is \textbf{$\sigma$-finite} if $E$ is the union of a countable collection of measurable sets, each of which has finite measure. 
	\end{definition}
	...
	\begin{lemma}[Finiteness Hierarchy]
		A finite measure space is $\sigma$-finite, and a $\sigma$-finite measure space is semifinite. 
	\end{lemma}
	\begin{proof}
  ...
  \end{definition}
	...
	If $\mu(E) = 0$ and $F \subset E$, then $\mu(F) = 0$ by monotonicity provided that $F \in \mathcal{M}$, but it need not be true that $F \in \mathcal{M}$. Such measures that contain all subsets of measure-$0$ sets in its domain are nice to work with. 

	\begin{definition}[Complete Measure]
		A measure space $(X, \mathcal{M}, \mu)$ is called \textbf{complete} if for all measure-$0$ sets $E \in \mathcal{M}$ and $F \subset E$, it follows that $F \in \mathcal{M}$. 
	\end{definition}

	\begin{definition}[Completion of a Measure]
		Let $(X, \mathcal{M}, \mu)$ be a measure space, and define 
		\begin{equation}
			\overline{\mathcal{M}} = \{E \cup F \colon E \in \mathcal{M}, \; F \subset N \text{ where } \mu(N) = 0\}
		\end{equation}
		Then, 
		\begin{enumerate}
			\item $\overline{\mathcal{M}}$ is a complete measure space, and 
			\item there is a unique extension $\overline{\mu}$ of $\mu$ to a complete measure on $\overline{\mathcal{M}}$. 
		\end{enumerate}
	\end{definition}

\subsection{Outer Measures}

	We can see that by taking a family $\mathcal{E}$ of elementary sets, on which a notion of measure is defined (such as rectangles), we can approximate arbitrary sets ``from the outside'' by countable unions of members of $\mathcal{E}$. This construction motivates the following definition. 
  ...
  \begin{definition}[Outer Measure]
    ...
    \begin{enumerate}
      ...
      \item \textit{Countable Subadditivity}. For arbitrary subsets $S, S_1, S_2, \ldots \subset X$, 
      \begin{equation}
        S \subset \bigcup_{k=1}^\infty S_k \implies \mu^\ast (S) \leq \sum_{k=1}^\infty \mu^\ast (S_k)
      \end{equation} 
    \end{enumerate}
  \end{definition}

  It's a hard definition, but a natural one, since we're taking all these elementary sets and trying to make them as snug as possible to define the outer measure of an arbitrary set. 

  \begin{theorem}[Outer Measure Induced by Elementary Measure]
    Let $\mathcal{E}$ be a collection of subsets of $X$ containing $\emptyset$ and $X$ itself, and the elementary measure $\mu: \mathcal{E} \to [0, +\infty]$ be such that $\mu(\emptyset) = 0$. Then, the function $\mu^\ast : 2^X \to [0, +\infty]$ defined 
    \begin{equation}
			\mu^\ast(\emptyset) = 0, \quad \mu^\ast (S) \coloneqq \inf \bigg\{ \sum_{k=1}^\infty \mu(E_k) \colon S \subset \bigcup_k E_k, \; E_k \in \mathcal{E} \bigg\}
    \end{equation}
		is an outer measure, called the \textbf{outer measure induced by $\mu$}. 
  \end{theorem}
  \begin{proof}
		We can see that by definition $\mu^\ast (\emptyset) = 0$. Now for each $S_k \subset X$, consider the covering $S_k \subset \bigcup_j E_{kj}$. Then, this implies that 
		\begin{equation}
			\mu^\ast (S_k) = \inf \bigg\{ \sum_{j=1}^\infty \mu(E_{kj}) \colon S_k \subset \bigcup_{j=1}^\infty E_{kj}, \; E_{kj} \in \mathcal{E} \bigg\}
		\end{equation}
		Therefore, 
		\begin{align}
			\sum_{k=1}^\infty \mu^\ast (S_k) 
				& = \sum_{k=1}^\infty \inf \bigg\{ \sum_{j=1}^\infty \mu(E_{kj}) \colon S_k \subset \bigcup_{j=1}^\infty E_{kj}, \; E_{kj} \in \mathcal{E} \bigg\} \\ 
				& \geq \inf \bigg\{ \sum_{k=1}^\infty \sum_{j=1}^\infty \mu(E_{kj}) \colon S_k \subset \bigcup_{j=1}^\infty E_{kj}, \; E_{kj} \in \mathcal{E} \bigg\} \\ 
				& \geq \inf \bigg\{ \sum_{j, k = 1}^\infty \mu(E_{kj}) \colon S \subset \bigcup_{j, k} E_{kj}, \; E_{kj} \in \mathcal{E} \bigg\} \\ 
				& = \mu^\ast (S)
		\end{align}
		where the final inequality follows from the fact that if $\{E_{kj}\}$ is an elementary covering of $\cup_k S_k$, then it is an elementary covering of $S$. Therefore, the latter infimum is taken over a larger set than the former. 
  \end{proof}
  ...
  \begin{definition}[Carathéodory Measure]
    Let $\mathcal{S}$ be a collection of subsets of $X$, $\mu: \mathcal{S} \to [0, +\infty]$ a set function, and $\mu^\ast$ the outer measure induced by $\mu$. 

		The measure $\overline{\mu}$ that is the restriction of $\mu^\ast$ to the $\sigma$-algebra $\mathcal{M}$ of $\mu^\ast$-measurable sets is called the \textbf{Carathéodory measure induced by $\mu$}. 
  \end{definition}
  ...
  \end{proof}
	...
	TBD: One might wonder why we don't use the inner measure? Let's define. 

\subsection{Premeasures}

	You can think of a premeasure as a more fundamental function that we can use to get an outer measure. 
...
\subsection{Measures on the Real Line}
	...
	Now we are ready to define a measure on the real line. There are two natural measures to construct, namely the \textit{Lebesgue measure} and the \textit{Borel measure}. 

\subsubsection{Carathéodory Construction}

	We can now define a measure on the real line, called the \textit{Lebesgue measure}. While we have all the machinery to define the Lebesgue measure in one shot, it is worthwhile to go through the construction again without depending on the more general theorems. We will do the following. We take a set function $\ell$ that assigns lengths to all intervals in $\mathbb{R}$. We have used this length to define the Lebesgue outer measure as an outer approximation of these intervals. We then use the outer measure to define measurable sets with Caratheodory's criterion, which states that measurable sets should split any set nicely into measurable sets. We verify that the collection of all measurable sets is a $\sigma$-algebra. Finally, we define the Lebesgue measure as the restriction of the Lebesgue outer measure to the collection of mesaurable sets. 
  ...
  \end{definition} 
	...
	We first first check that this is indeed a generalization of Jordan measure, in the sense that if $E$ is Jordan measurable, then its Lebesgue outer measure is the same as its Jordan measure. 
  ...
  \end{proof}
	...
	Finally, we can define the Lebesgue measure by restricting our outer measure to measurable sets. 
  ...
  We talk about how Lebesgue measurable sets interact with other common sets, such as open and closed sets in a topology. 
  ...
  Depending on the context, it is helpful to use one definition over another when proving measurability. Just remember that the Carathéodory definition is the most general, since it doesn't even assume a topology on a space, and that is the definition that we will use by default. This next theorem is a different flavor of Littlewood's first principle. It tells us that we can use a finite union of intervals that ``symmetrically'' approximates measurable sets on the real line. 
...
\subsubsection{Borel Construction}
	...
	Another natural choice is the \textit{Borel measure}, which is very easy to define now. 

	\begin{definition}[Borel Measure]
		The \textbf{Borel measure} $\lambda_B$ on $\mathbb{R}$ is the 
	\end{definition}

	\begin{lemma}[Lebesgue Sigma Algebra is Slightly Bigger]
	  
	\end{lemma}

\subsubsection{Stieltjes Construction}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      ...
      \end{equation}
      \item $S$ is a countably infinite disjoint union of sets $\{S_j\}_{j=1}^\infty$, then 
      \begin{equation}
...
\subsection{Exercises} 
  ...
  \end{solution}
	...
	\begin{exercise}[Folland 1.1]
		A family of sets $\mathcal{R} \subset \mathcal{P}(X)$ is called a \textbf{ring} if it is closed under finite unions and differences (i.e., if $E_1, \dots, E_n \in \mathcal{R}$, then $\bigcup_1^n E_j \in \mathcal{R}$, and if $E, F \in \mathcal{R}$, then $E \setminus F \in \mathcal{R}$). A ring that is closed under countable unions is called a \textbf{$\sigma$-ring}.
	...
	\begin{solution}
		...
		\begin{enumerate}
		  \item Since 
				\begin{equation}
					E \cap F = (E \cup F) \setminus ((E \setminus F) \cup (F \setminus E)) = E \setminus (E \setminus F), 
				\end{equation}
				by induction the claim holds for rings. For $\sigma$-rings, we have 
				\begin{equation}
					\bigcap_{k=1}^\infty = E_1 \cap \bigg( \bigcap_{k=2}^\infty E_k \bigg) = E_1 \setminus \bigg( E_1 \setminus \bigcap_{k=2}^\infty E_k \bigg) = E_1 \setminus \bigg( \bigcup_{k=2}^\infty E_1 \setminus E_k \bigg)
				\end{equation}

			\item Let $\mathcal{R}$ be a ring. We prove bidirectionally. 
				\begin{enumerate}
					\item $(\rightarrow)$. Since $\mathcal{R}$ is an algebra, $\emptyset^c = X \in \mathcal{R}$. 
					\item $(\leftarrow)$. If $X \in \mathcal{R}$, complementation can be achieved by $E^c = X \setminus E \in \mathcal{R}$. 
				\end{enumerate}

				Let $\mathcal{R}$ be a $\sigma$-ring. 
				\begin{enumerate}
					\item Since $\mathcal{R}$ is a ring, for any $E \in \mathcal{R}, E \setminus E = \emptyset \in \mathcal{R}$. Since $\mathcal{R}$ is a $\sigma$-algebra, $\emptyset^c = X \setminus E \in \mathcal{R}$. 
					\item Same as above for rings. 
				\end{enumerate}

			\item Since $\mathcal{R}$, $\mathcal{A} = \{E \subset X \colon E \in \mathcal{R} \text{ or } E^c \in \mathcal{R} \}$ has complements, i.e. $E \in \mathcal{A} \implies E^c \in \mathcal{A}$. As for countable unions, let $E_1, E_2, \ldots \in \mathcal{A}$. Then, for each $E_k$, let $E_k^\prime = E_k$ or $E_k^c \in \mathcal{A}$, depending on which is in $\mathcal{R}$. Then $E_k^\prime \in \mathcal{R}$. Since $\mathcal{R}$ is a ring, $\bigcup_{k=1}^\infty E_k^\prime \in \mathcal{R}$. But each $E$ 
				\begin{align}
					\bigcup_k E_k & = \bigg( \bigcup_i E_i \bigg) \cup \bigg( \bigcup_j E_j \bigg) \\ 
												& = \bigg( \underbrace{\bigcup_i E_i}_{\in \mathcal{R}} \bigg) \cup \bigg( \underbrace{\bigcap_j E_j^c}_{\in \mathcal{R}} \bigg)^c
				\end{align}
				So, $\cup_i E_i \in \mathcal{R} \implies \cup_i E_i \in \mathcal{A}$, and $\cap_j E_j^c \in \mathcal{R} \implies \big( \cap_j E_j^c \big)^c \in \mathcal{A}$. The union of these two sets are in $A$. 

			\item Let $\mathcal{A} = \{E \subset X \colon E \cap F \in \mathcal{R} \text{ for all } F \in \mathcal{R} \}$. If $E \in \mathcal{A}$, $E \cap F \in \mathcal{R}$ for all $F \in \mathcal{R} \implies E \setminus (E \setminus F) \in \mathcal{R}$ for all $F \in \mathcal{R}$. We just need to prove $X \in \mathcal{A}$, which is trivial since $X \cap F = F \in \mathcal{R}$ for all $F \in \mathcal{R}$. 
		\end{enumerate}
	\end{solution}

	\begin{exercise}[Folland 1.2]
	...
	\begin{exercise}[Folland 1.4]
		An algebra $\mathcal{A}$ is a $\sigma$-algebra iff $\mathcal{A}$ is closed under countable increasing unions (i.e., if $\{E_j\}_1^\infty \subset \mathcal{A}$ and $E_1 \subset E_2 \subset \cdots$, then $\bigcup_1^\infty E_j \in \mathcal{A}$).
	\end{exercise}
	\begin{solution}
		The right implication is trivial since countable increasing unions is still a countable union. For the left implication, let $\{E_k\}$ be a countable set of elements in algebra $\mathcal{A}$. Then, set 
		\begin{equation}
			E_1^\prime = E_1, \quad E_k^\prime = \bigcup_{j=1}^k E_j
		\end{equation}
		Then, $E_k^\prime$ are increasing, which implies that 
		\begin{equation}
			\bigcup_{k=1}^\infty E_k = \bigcup_{k=1}^\infty E_k^\prime \in \mathcal{A}
		\end{equation}

	\end{solution}

	\begin{exercise}[Folland 1.5]
	...
	\end{exercise}
	\begin{solution}
		Let $\mathcal{S}$ be a collection of subsets of $X$. FOr each $A \in \sigma(\mathcal{S})$, there exists a countable subfamily $\mathcal{S}_0 \subset \mathcal{S}$ s.t. $\mathcal{A} \in \sigma(\mathcal{S}_0)$. Consider set\footnote{I tried to first create \textit{one} countable subset $\mathcal{F}$ and proceed from there, but the problem is that we need to work with a possibly uncountable union of these countable subsets.} 
		\begin{equation}
			\mathcal{C} = \{B \in \sigma(\mathcal{S}) \colon \text{for some countable } \mathcal{S}_0 \subset \mathcal{S}, \; B \in \sigma(\mathcal{S}_0) \}
		\end{equation}
		Clearly, $\mathcal{S} \subset \mathcal{C}$ since for any $B \in \mathcal{S}$, set $\mathcal{S}_0 = \{B\}$. If $B \in \mathcal{C}$, then $B^c \in \mathcal{C}$ trivially. Now let $B_k \in \mathcal{C}$. Then for each $B_k$, there exists some countable $\mathcal{S}_k \subset \mathcal{S}$ s.t. $B_k \in \sigma(\mathcal{S}_k)$. So for $B = \bigcup_{k=1}^\infty B_k$, there exists countable $\bigcup_{k=1}^\infty \mathcal{S}_k \subset \mathcal{S}$ s.t. $B \in \sigma \big( \bigcup_{k=1}^\infty \mathcal{S}_k \big) \implies B \in \mathcal{C}$. Therefore, $\mathcal{C}$ is a $\sigma$-algebra. 

		Also, if $B \in \mathcal{C}$, we have $B \in \sigma(\mathcal{S}_0) \subset \sigma(\mathcal{S}) \implies C \subset \sigma(\mathcal{S})$. So, $\mathcal{S} \subset \mathcal{C} \subset \sigma(\mathcal{S})$, and by definition of the generated $\sigma$-algebra being minimal, $\mathcal{C} = \sigma(\mathcal{S})$. Therefore, $\sigma(\mathcal{E}) = \mathcal{C}$, which is the union of $\sigma$-algebras generated by countable subsets of $\mathcal{E}$. 
	\end{solution}

	\begin{exercise}[Folland 1.7]
	...
	\begin{solution}
		...
		\end{equation}
		Given pairwise disjoint sets $\{E_k\}$, 
		\begin{equation}
			(\mu + \nu) \bigg( \bigsqcup_{k=1}^\infty E_k \bigg) = \mu \bigg( \bigsqcup_{k=1}^\infty E_k \bigg) + \nu \bigg( \bigsqcup_{k=1}^\infty E_k \bigg) = \sum_{k=1}^\infty \mu(E_k) + \sum_{k=1}^\infty \nu(E_k)
		\end{equation}
		Both series converge (absolutely) in $[0, +\infty]$, so we can sum the terms 
		\begin{equation}
			= \sum_{k=1}^\infty (\mu + \nu)(E_k)
		\end{equation}
		By induction, finite sums follow. 
	\end{solution}

	\begin{exercise}[Folland 1.8]
		If $(X, \mathcal{M}, \mu)$ is a measure space and $\{E_j\}_1^\infty \subset \mathcal{M}$, then 
		\begin{equation}
			\mu(\liminf E_j) \le \liminf \mu(E_j).
		\end{equation}
		Also, 
		\begin{equation}
			\mu(\limsup E_j) \ge \limsup \mu(E_j)
		\end{equation}
		provided that $\mu(\bigcup_1^\infty E_j) < \infty$.
	\end{exercise}
	\begin{solution}
		We have\footnote{I tried to use countable subadditivity, but this is too restrictive of an inequality.}
		\begin{align}
			\mu \big( \liminf E_n \big) & = \mu \bigg( \bigcup_{n=1}^\infty \bigcap_{m=n}E_n \bigg) \\ 
																	& = \lim_{n \to \infty} \mu \bigg( \bigcap_{m=n} E_n \bigg) \\ 
																	& \leq \lim_{n \to \infty} \inf_{m \geq n} \mu(E_m) 
		\end{align}
	\end{solution}

	\begin{exercise}[Folland 1.9]
	...
	\end{exercise}
	\begin{solution}
		We know that $E \cup F = (E \cap F) \sqcup (E \setminus F) \sqcup (F \setminus E)$, so 
		\begin{equation}
		  \mu(E \cup F) = \mu(E \cap F) + \mu(E \setminus F) + \mu(F \setminus E)
		\end{equation}
		By adding $\mu(E \cap F)$ to both sides, we have 
		\begin{align}
			\mu(E \cup F) + \mu(E \cap F) & = \big( \mu(E \cap F) + \mu(E \cap F) \big) + \big( \mu(F \setminus E) + \mu(E \cap F) \big) \\ 
																		& = \mu(E) + \mu(F)
		\end{align}
	\end{solution}

	\begin{exercise}[Folland 1.10]
	...
	\end{exercise}
	\begin{solution}
		We have $\mu_E (\emptyset) = \mu(\emptyset \cap E) = \mu(\emptyset) = 0$. To prove countable addivitiy, let $\{A_j\}$ be a countable set of disjoint sets. Then, 
		\begin{align}
			\mu_E \bigg( \bigsqcup_j A_j \bigg) & = \mu \bigg( \Big( \bigsqcup_{j} A_j \Big) \cap E \bigg) \\ 
																					& = \mu \bigg( \bigsqcup_j (A_j \cap E) \bigg) \\ 
																					& = \sum_j \mu(A_j \cap E) \\ 
																					& = \sum_j \mu_E (A_j)
		\end{align}
	\end{solution}

	\begin{exercise}[Folland 1.11]
		A finitely additive measure $\mu$ is a measure iff it is continuous from below. If $\mu(X) < \infty$, $\mu$ is a measure iff it is continuous from above.
	\end{exercise}
	\begin{solution}
		We prove the first claim bidirectionally, note that the forward implication is obvious from the previous theorem. For the reverse, let $\{E_j\}_{j=1}^\infty$ be disjoint subsets. 
	\end{solution}

	\begin{exercise}[Folland 1.12]
	...
	\begin{exercise}[Folland 1.15]
		Given a measure $\mu$ on $(X, \mathcal{M})$, define $\mu_0$ on $\mathcal{M}$ by 
		\begin{equation}
			\mu_0(E) = \sup\{\mu(F) : F \subset E \text{ and } \mu(F) < \infty\}.
		\end{equation}
		\begin{enumerate}[label=\textbf{\alph*.}]
	...
	\begin{exercise}[Folland 1.16]
		Let $(X, \mathcal{M}, \mu)$ be a measure space. A set $E \subset X$ is called \textbf{locally measurable} if $E \cap A \in \mathcal{M}$ for all $A \in \mathcal{M}$ such that $\mu(A) < \infty$. Let $\widetilde{\mathcal{M}}$ be the collection of all locally measurable sets. Clearly $\mathcal{M} \subset \widetilde{\mathcal{M}}$; if $\mathcal{M} = \widetilde{\mathcal{M}}$, then $\mu$ is called \textbf{saturated}.
		\begin{enumerate}[label=\textbf{\alph*.}]
			...
			\item Suppose that $\mu$ is semifinite. For $E \in \widetilde{\mathcal{M}}$, define $\underline{\mu}(E) = \sup\{\mu(A) : A \in \mathcal{M} \text{ and } A \subset E\}$. Then $\underline{\mu}$ is a saturated measure on $\widetilde{\mathcal{M}}$ that extends $\mu$.
			\item Let $X_1, X_2$ be disjoint uncountable sets, $X = X_1 \cup X_2$, and $\mathcal{M}$ the $\sigma$-algebra of countable or co-countable sets in $X$. Let $\mu_0$ be counting measure on $\mathcal{P}(X_1)$, and define $\mu$ on $\mathcal{M}$ by $\mu(E) = \mu_0(E \cap X_1)$. Then $\mu$ is a measure on $\mathcal{M}$, $\widetilde{\mathcal{M}} = \mathcal{P}(X)$, and in the notation of parts (c) and (e), $\widetilde{\mu} \neq \underline{\mu}$.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 1.17]
		If $\mu^*$ is an outer measure on $X$ and $\{A_j\}_1^\infty$ is a sequence of disjoint $\mu^*$-measurable sets, then 
		\begin{equation}
	...
	\begin{exercise}[Folland 1.19]
		Let $\mu^*$ be an outer measure on $X$ induced from a finite premeasure $\mu_0$. If $E \subset X$, define the \textbf{inner measure} of $E$ to be $\mu_*(E) = \mu_0(X) - \mu^*(E^c)$. Then $E$ is $\mu^*$-measurable iff $\mu^*(E) = \mu_*(E)$. (Use Exercise 18.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 1.20]
		Let $\mu^*$ be an outer measure on $X$, $\mathcal{M}^*$ the $\sigma$-algebra of $\mu^*$-measurable sets, $\overline{\mu} = \mu^*|\mathcal{M}^*$, and $\mu^+$ the outer measure induced by $\overline{\mu}$ as in (1.12) (with $\overline{\mu}$ and $\mathcal{M}^*$ replacing $\mu_0$ and $\mathcal{A}$).
		\begin{enumerate}[label=\textbf{\alph*.}]
	...
	\begin{exercise}[Folland 1.21]
		Let $\mu^*$ be an outer measure induced from a premeasure and $\overline{\mu}$ the restriction of $\mu^*$ to the $\mu^*$-measurable sets. Then $\overline{\mu}$ is saturated. (Use Exercise 18.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 1.22]
		Let $(X, \mathcal{M}, \mu)$ be a measure space, $\mu^*$ the outer measure induced by $\mu$ according to (1.12), $\mathcal{M}^*$ the $\sigma$-algebra of $\mu^*$-measurable sets, and $\overline{\mu} = \mu^*|\mathcal{M}^*$.
		\begin{enumerate}[label=\textbf{\alph*.}]
	...
	\begin{exercise}[Folland 1.23]
		...
		\begin{enumerate}[label=\textbf{\alph*.}]
			...
			\item Define $\mu_0$ on $\mathcal{A}$ by $\mu_0(\varnothing) = 0$ and $\mu_0(A) = \infty$ for $A \neq \varnothing$. Then $\mu_0$ is a premeasure on $\mathcal{A}$, and there is more than one measure on $\mathcal{P}(\mathbb{Q})$ whose restriction to $\mathcal{A}$ is $\mu_0$.
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 1.29]
		Let $E$ be a Lebesgue measurable set.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item If $E \subset N$ where $N$ is the nonmeasurable set described in \S 1.1, then $m(E) = 0$.
			\item If $m(E) > 0$, then $E$ contains a nonmeasurable set. (It suffices to assume $E \subset [0, 1]$. In the notation of \S 1.1, $E = \bigcup_{r \in R} E \cap N_r$.)
		\end{enumerate}
	\end{exercise}
	\begin{solution}
	...
	\begin{exercise}[Folland 1.30]
		If $E \in \mathcal{L}$ and $m(E) > 0$, for any $\alpha < 1$ there is an open interval $I$ such that 
		\begin{equation}
			m(E \cap I) > \alpha m(I).
		\end{equation}
	\end{exercise}
	...
	\begin{exercise}[Folland 1.31]
		If $E \in \mathcal{L}$ and $m(E) > 0$, the set $E - E = \{x - y : x, y \in E\}$ contains an interval centered at $0$. (If $I$ is as in Exercise 30 with $\alpha > \frac{3}{4}$, then $E - E$ contains $(-\frac{1}{2}m(I), \frac{1}{2}m(I))$.)
	\end{exercise}
	\begin{solution}

	\end{solution}

	\begin{exercise}[Folland 1.32]
		Suppose $\{\alpha_j\}_1^\infty \subset (0, 1)$.
		\begin{enumerate}[label=\textbf{\alph*.}]
			\item $\prod_1^\infty (1 - \alpha_j) > 0$ iff $\sum_1^\infty \alpha_j < \infty$. (Compare $\sum_1^\infty \log(1 - \alpha_j)$ to $\sum \alpha_j$.)
	...
	\begin{exercise}[Folland 1.33]
		There exists a Borel set $A \subset [0, 1]$ such that 
		\begin{equation}
			0 < m(A \cap I) < m(I)
		\end{equation}
		for every subinterval $I$ of $[0, 1]$. (Hint: Every subinterval of $[0, 1]$ contains Cantor-type sets of positive measure.)
	\end{exercise}
	\begin{solution}

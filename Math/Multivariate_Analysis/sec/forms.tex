...
\subsection{Line Integrals}
  ...
  If $C$ is an oriented simple curve or an oriented simple closed curve, then we can unambiguously define line integrals along them. 
	...
	\begin{definition}[Reparamaterization of Paths]
		Let $h: [\alpha, \beta] \to [a, b]$ be a smooth bijective monotonic function.\footnote{So it takes endpoints to endpoints.} Given an oriented simple path function $p: [a,b]\subset \mathbb{R} \longrightarrow \mathbb{R}^n$, the composition
		\begin{equation}
			\rho = p \circ h: [\alpha, \beta] \longrightarrow \mathbb{R}^n
		\end{equation}
		is called a \textbf{reparamaterization} of $p$. 
		\begin{enumerate}
		  \item If $h$ preserves the direction in which the path travels, i.e. if $(p \circ h)(\alpha) = a \text{ and } (p \circ h)(\beta) = b$ then $h$ is \textit{orientation preserving}. 
			\item If $(p \circ h)(\alpha) = b \text{ and } (p \circ h)(\beta) = a$, then $h$ is \textit{orientation reversing}. 
		\end{enumerate}
		...
		\begin{figure}[H]
			...
			\begin{subfigure}[b]{0.47\textwidth}
				...
				\begin{tikzpicture}[
					scale=1.2,
					forward arrows/.style={
						postaction={decorate},
						decoration={markings,
							mark=at position 0.25 with {\arrow[scale=1.2]{>}},
							mark=at position 0.55 with {\arrow[scale=1.2]{>}},
							mark=at position 0.85 with {\arrow[scale=1.2]{>}}
						}
					}
				]
					\fill (0,0) circle (1.5pt) node[below left] {$a$};
					\fill (2.5,1.5) circle (1.5pt) node[right] {$b$};
					
					\draw[thick, forward arrows] (0,0) to[out=15, in=195] (2.5,1.5);
					
					\node at (1.25, -0.6) {preserving};
				\end{tikzpicture}
				\caption{Orientation preserving path.}
				\label{fig:path_preserving}
			\end{subfigure}
			\hfill 
			\begin{subfigure}[b]{0.47\textwidth}
				\centering
				\begin{tikzpicture}[
					...
					\draw[thick, backward arrows] (0,0) to[out=15, in=195] (2.5,1.5);
					
					\node at (1.25, -0.6) {reversing};
				\end{tikzpicture}
				\caption{Orientation reversing path.}
				\label{fig:path_reversing}
			\end{subfigure}
			\caption{Comparison of orientation preserving and reversing paths between points $a$ and $b$.}
			\label{fig:orientation_paths}
		\end{figure}
  \end{definition}

	Note that a path $c$ having the same image as $p$ does not imply that $c$ is a reparamaterization of $p$, since $c$ may not be injective. 
...
\subsubsection{2-Dimensional Paramaterizations of Surfaces}
  ...
  \begin{definition}[Tangent Vectors of Surfaces Embedded in $\mathbb{R}^3$]
		...
		\begin{equation}
			\varphi: \mathbb{R}^2 \longrightarrow \mathbb{R}^3, \;\;\; \varphi(u, v) \equiv \begin{pmatrix} x (u, v) \\ y(u, v) \\ z(u, v) \end{pmatrix}
		\end{equation}
  ...
  \begin{definition}[Regular Surfaces]
		...
		\end{center}
		It is quite clear that $(\frac{\partial \varphi}{\partial u} \times \frac{\partial \varphi}{\partial v})(u_0, v_0) \neq 0 \implies \frac{\partial \varphi}{\partial u}$ and $\frac{\partial \varphi}{\partial v}$ are linearly independent. This means that an entire span of tangent vectors, i.e. a tangent plane, of the surface $S$ at $\varphi(u_0, v_0)$ exists. $S$ is said to be \textit{regular} if it is regular at all points $\varphi(u_0, v_0) \in S$. 
  \end{definition}
  ...
  We finally construct the concept of signed areas before defining surface integration. We have all the tools we need to calculate surface areas, but remember that integration also covers the concept of \textit{signed areas}, which could be negative. In order to define this, we define the concept of orientation on surfaces. 
...
\subsubsection{Stokes' Theorem}
  ...
  \begin{theorem}[Stokes' Theorem]
		Let $S$ be an oriented regular surface defined by paramaterization $\varphi: D \subset \mathbb{R}^2 \longrightarrow \mathbb{R}^3$, and let the image of the boundary $\partial D$ under $\varphi$ be the boundary $\partial S$ of $S$. 

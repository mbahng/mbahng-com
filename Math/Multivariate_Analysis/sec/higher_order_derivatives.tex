	    ...
	    \centering
			\resizebox{\linewidth}{!}{%
				\begin{tikzpicture}[scale=0.9]
					...
					\fill (1,1) circle (2pt) node[above right] {$x$};
					\draw[->, thick] (1,1) -- (-3,0) node[below left] {$x+h$};
				\end{tikzpicture}%
			}
	    \caption{}
	  \end{subfigure}
	    ...
	    \centering
			\resizebox{\linewidth}{!}{%
				\begin{tikzpicture}[scale=0.9]
					...
					\fill (-2,3) circle (2pt) node[above left] {$f(x)$};
					\draw[blue, ->, thick] (-2,3) -- (5.5,4.75);
					\draw[red, very thick, ->]
						(-2,3)
						.. controls (0.1,3.68) and (2.5,1.35) ..
						(4,-1);
					\draw[black, very thick, ->]
						(-2,3)
						.. controls (0.1,3.68) and (1.9,2.0) ..
						(2.6,1.25)
						.. controls (3.3,0.2) and (4.35,-0.35) ..
						(5,-2)
						node[below right] {$f(x+h)$};
				\end{tikzpicture}%
			}
	    \caption{}
	  \end{subfigure}
		\caption{As we change $x \to x + h$, the actual function deviates from $f(x)$ to $f(x + h)$. The first order approximation (blue) and second order approximation (red) are shown.}
	  \label{fig:taylor_domain_codomain_displacement}
	\end{figure}

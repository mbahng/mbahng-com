  ...
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \begin{tikzpicture}[scale=0.55]
      \begin{axis}[
        ...
        % Opaque context shell
        \addplot3[
          surf, domain=-1.5:1.5, domain y=0:360, samples=30, samples y=20,
          z buffer=sort, colormap/viridis, faceted color=none, opacity=0.6
        ] (
        ...
        % X cross-sections (Slices perpendicular to X-axis)
        % x = -0.8
        \addplot3[fill=blue!50, draw=blue, thick, domain=0:360, samples=40, z buffer=sort] 
          ({-0.8}, {1.207 * cos(x)}, {1.207 * sin(x)});
        % x = 0.0
        \addplot3[fill=red!50, draw=red, thick, domain=0:360, samples=40, z buffer=sort] 
          ({0.0}, {0.474 * cos(x)}, {0.474 * sin(x)});
        % x = 0.8
        \addplot3[fill=teal!50, draw=teal, thick, domain=0:360, samples=40, z buffer=sort] 
          ({0.8}, {1.207 * cos(x)}, {1.207 * sin(x)});
      \end{axis}
    \end{tikzpicture}
    \caption{Cross sections w.r.t $x$-axis}
    \label{fig:cross_x}
  \end{subfigure}
  \hfill 
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \begin{tikzpicture}[scale=0.55]
      \begin{axis}[
        ...
        % Y cross-sections using a mathematical filter constraint
        % y = -0.5
        \addplot3[fill=blue!50, draw=blue, thick, domain=0:360, domain y=-1.5:1.5, samples=40, samples y=2, z buffer=sort, restrict y to domain=-0.51:-0.49] 
          ({sqrt((2.25 - y^2) * (0.1 + 0.7 * y^2)) * cos(x)}, {-0.5}, {y});
        % y = 0.0
        \addplot3[fill=red!50, draw=red, thick, domain=0:360, domain y=-1.5:1.5, samples=40, samples y=2, z buffer=sort, restrict y to domain=-0.01:0.01] 
          ({sqrt((2.25 - y^2) * (0.1 + 0.7 * y^2)) * cos(x)}, {0.0}, {y});
        % y = 0.5
        \addplot3[fill=teal!50, draw=teal, thick, domain=0:360, domain y=-1.5:1.5, samples=40, samples y=2, z buffer=sort, restrict y to domain=0.49:0.51] 
          ({sqrt((2.25 - y^2) * (0.1 + 0.7 * y^2)) * cos(x)}, {0.5}, {y});
      \end{axis}
    \end{tikzpicture}
    \caption{Cross sections w.r.t $y$-axis}
    \label{fig:cross_y}
  \end{subfigure}
  \hfill 
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \begin{tikzpicture}[scale=0.55]
      \begin{axis}[
        ...
        \addplot3[
          surf, domain=-1.5:1.5, domain y=0:360, samples=30, samples y=20,
          z buffer=sort, colormap/viridis, faceted color=none
        ] (
          {sqrt((2.25 - x^2) * (0.1 + 0.7 * x^2)) * cos(y)},
          {sqrt((2.25 - x^2) * (0.1 + 0.7 * x^2)) * sin(y)},
          {x}
        ...
        % Z cross-sections (Slices perpendicular to Z-axis)
        % z = 0.0 (Waist)
        \addplot3[fill=red!50, draw=red, thick, domain=0:360, samples=40, z buffer=sort] 
          ({0.4743 * cos(x)}, {0.4743 * sin(x)}, {0});
        % z = 1.0 (Bulge)
        \addplot3[fill=blue!50, draw=blue, thick, domain=0:360, samples=40, z buffer=sort] 
          ({1.0 * cos(x)}, {1.0 * sin(x)}, {1.0});
        % z = 1.3 (Tip)
        \addplot3[fill=teal!50, draw=teal, thick, domain=0:360, samples=40, z buffer=sort] 
          ({0.8476 * cos(x)}, {0.8476 * sin(x)}, {1.3});
      \end{axis}
    \end{tikzpicture}
    \caption{Cross sections w.r.t $z$-axis}
    \label{fig:cross_z}
  \end{subfigure}
  \caption{3D Geometric Cross Sections of a Peanut-Shaped Surface along Three Principal Axes}
  \label{fig:peanut_slices}
\end{figure}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot3[
      ...
      z buffer=sort,
      colormap/viridis,
      opacity=0.12, % Highly translucent to emphasize the slices
      shader=faceted
    ] (
      {sqrt((2.25 - x^2) * (0.1 + 0.7 * x^2)) * cos(y)},
      {sqrt((2.25 - x^2) * (0.1 + 0.7 * x^2)) * sin(y)},
    ...
    % Text labels placed in 3D space near each slice
    \node[color=red, anchor=west] at (axis cs:0.6,0,0) {$z = 0.0$ (Waist)};
    \node[color=blue, anchor=west] at (axis cs:1.1,0,1.0) {$z = 1.0$ (Bulge)};
    \node[color=teal, anchor=west] at (axis cs:1.0,0,1.3) {$z = 1.3$ (Tip)};
...
\subsection{Examples} 
  ...
  We now work through several representative examples. In each category, we first do an easier example with a polynomial-type integrand and then a harder example with logarithmic or exponential terms. 

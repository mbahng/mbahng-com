...
\subsection{Dense Subsets}
  ...
  \end{example}
	...
	\begin{theorem}[Irrational Multiples are Dense in $[0, 1]$]
		\label{thm:irrational_dense_in_unit}
		In $x, y \in [0, 1]$, let us define $x + y \coloneqq \{ x + y\}$, where $x = \lfloor x \rfloor + \{x\}$, i.e. it is the mod 1 function. Then, for any irrational $\lambda \in \mathbb{R} \setminus \mathbb{Q}$, the set 
		\begin{equation}
			S = \{ \lambda t \colon t \in \mathbb{Z} \}
		\end{equation}
		is dense in $[0, 1]$. 
	\end{theorem}
	\begin{proof}
		WLOG, we can assume that $\lambda \in (0, 1)$ since if it wasn't, then we can write each element as $\{(z + \lambda)t\} = \{zt + \lambda t\} = \{\lambda t\}$ since $zt \in \mathbb{Z}$, and our calculations would not change. Now choose some $N \in \mathbb{N}$ and divide $[0, 1]$ into $N$ equal intervals. Choose the $N + 1$ points 
		\begin{equation}
			\{0\}, \{\lambda\}, \{2 \lambda\}, \{3 \lambda\}, \ldots, \{N \lambda\} 
		\end{equation}
		By the pigeonhole principle, there are two numbers $a \neq b$ s.t. $\{\lambda a\}, \{\lambda b\}$ are in the same interval, so $|\lambda a - \lambda b| < \frac{1}{N}$. We can split 
		\begin{align}
			\lambda b & = \lfloor \lambda b \rfloor + \{ \lambda b \} \\ 
			\lambda a & = \lfloor \lambda a \rfloor + \{ \lambda a \} 
		\end{align} 
		which implies that 
		\begin{equation}
			|(b - a) \lambda| = |( \lfloor \lambda b \rfloor - \lfloor \lambda a \rfloor )| + |( \{\lambda b\} - \{ \lambda a\} )|
		\end{equation} 
		We have two cases. 
		\begin{enumerate}
			\item If $\{\lambda b \} - \{\lambda a\} < \frac{1}{N}$, then 
				\begin{equation}
					(b - a) \lambda = (\lfloor \lambda b \rfloor - \lfloor \lambda a \rfloor) + (\{\lambda b \} - \{\lambda a\}) < (\lfloor \lambda b \rfloor - \lfloor \lambda a \rfloor) + \frac{1}{N}
				\end{equation}
				By modding 1, we get $\{(b - a) \lambda\} < \frac{1}{N}$. 

			\item If $\{\lambda a\} - \{\lambda b\} < \frac{1}{N}$, then $\{\lambda b\} - \{\lambda a\} > - \frac{1}{N}$, which implies that $\{ \{\lambda b\} - \{\lambda a\} \} > 1 - \frac{1}{N}$. Therefore, 
				\begin{equation}
					(b - a) \lambda = (\lfloor \lambda b \rfloor - \lfloor \lambda a \rfloor) + (\{\lambda b \} - \{\lambda a\}) > (\lfloor \lambda b \rfloor - \lfloor \lambda a \rfloor) + 1 - \frac{1}{N}
				\end{equation}
				By modding 1, we get $\{(b - a) \lambda\} > 1 - \frac{1}{N} \implies 1 - \{(b - a) \lambda\} < \frac{1}{N}$. 
		\end{enumerate} 
		Now let $q = b - a \in \mathbb{Z}$, and we will let $q\lambda$ be our generating number. Since $0 < q \lambda < \frac{1}{N}$ (similar tarugment for when $1 - \frac{1}{N} < q < 1$), by continuously adding them, are guaranteed to hit every interval. That is, the set 
		\begin{equation}
			\{k (b - a) \lambda \colon k \in \mathbb{Z}\}
		\end{equation} 
		will intersect every interval at least once. Since $N$ was chosen arbitrarily, we are done.  
	\end{proof}

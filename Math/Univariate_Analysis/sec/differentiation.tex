  ...
  \begin{definition}[Derivative]
    ...
    \begin{enumerate}
      ...
      \item \textit{Fundamental Increment Lemma}. If there exists a number $f^\prime (x) \in \mathbb{R}$ s.t. 
        ...
        \end{equation}
				We call the linear mapping $df_x: \mathbb{R} \to \mathbb{R}$ defined $df_x (h) = f^\prime (x) h$ as the \textbf{differential} of $f$ at $x$. 
...
\subsection{Rules of Differentiation}
	...
	\begin{lemma}[Linearity of Differentiation]
		\label{thm:linearity_differentiation}
		If $f, g: E \to \mathbb{R}$ are differentiable at $x$, then $f + g$ is differentiable at $x$ with 
		\begin{equation}
	...
	\begin{lemma}[Product Rule]
		...
		If $f, g: E \to \mathbb{R}$ are differentiable at $x$, then $fg$ is differentiable at $x$ with 
		\begin{equation}
			(fg)^\prime (x) = f^\prime (x) g(x) + f(x) g^\prime (x)
		\end{equation}
	\end{lemma}
	\begin{proof}
		For products, let's not take the quotient just yet. 
		\begin{equation}
	...
	\begin{lemma}[Quotient Rule]
		...
		If $f, g: E \to \mathbb{R}$ are differentiable at $x$ and $g(x) \neq 0$ in a neighborhood of $x$, then $f/g$ is differentiable at $x$ with 
		\begin{equation}
			\bigg(\frac{f}{g} \bigg)^\prime (x) = \frac{f^\prime(x) g(x) - f(x) g^\prime (x)}{g(x)^2}
		\end{equation}
	\end{lemma}
	\begin{proof}
		We will prove that $(1/g)^\prime (x) = - \frac{g^\prime (x)}{g(x)^2}$. Using the limit rules, 
		\begin{align}
			\bigg(\frac{1}{g} \bigg)^\prime & = \lim_{y \to x} \frac{(1/g)(x) - (1/g)(y)}{x - y} \\
																			& = \lim_{y \to x} \frac{g(y) - g(x)} {(x - y) g(x) g(y)}  \\ 
																			& = \bigg( \lim_{y \to x} \frac{g(y) - g(x)}{x - y} \bigg) \bigg( \lim_{y \to x} \frac{1}{g(x) g(y)}\bigg) \\ 
																			& = -g^\prime (x) \cdot \frac{1}{g(x)^2}
		\end{align} 
		Therefore, 
		\begin{align}
			\bigg(\frac{f}{g} \bigg)^\prime (x) & = f(x) \bigg(\frac{1}{g})^\prime (x) + f^\prime (x) \bigg(\frac{1}{g}\bigg)(x) \\ 
																					& -\frac{f(x) g^\prime (x)}{(g(x))^2} + \frac{f^\prime (x)}{g(x)} \\ 
  ...
  \begin{theorem}[Chain Rule][thm:chain]
  ...
  \begin{theorem}[Inverse Function Theorem][thm:inverse-ft]
    Let $E_1, E_2 \subset \mathbb{R}$ and $f: E_1 \to E_2$ be a \hyperref[pst-def:homeomorphism]{homeomorphism}. Let $x_0 \in E_1$ be a limit point of $E_1$, and let $y_0 = f(x_0)$. 
    \begin{enumerate}
      \item If $f$ is differentiable at $x_0$ and $f^\prime (x_0) \neq 0$, then $f^{-1}$ is differentiable at $y_0$ (which is a limit point of $E_2$), 
      \item Furthermore, 
        \begin{equation}
          (f^{-1})^\prime(y_0) = \frac{1}{f^\prime(x_0)} \iff df^{-1}(y_0) = \big(df(x_0)\big)^{-1}
        \end{equation}
    \end{enumerate}
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}[scale=1.2]
        ...
        \begin{scope}
          \draw[->] (-0.5,0) -- (3,0) node[right] {$E_1$};
          \draw[->] (0,-0.5) -- (0,3) node[above] {$E_2$};
          
          % Curve for f (y = 0.2*x^2 + 0.5)
          \draw[thick, domain=0:2.5, samples=100] plot (\x, {0.2*\x*\x + 0.5});
          \node at (2.2,2.2) {$f$};
          
          % Point x0 = 2, y0 = 1.3
          \coordinate (P1) at (2, 1.3);
          \fill (P1) circle (0.06);
          \draw[dashed, gray] (2,0) -- (2,1.3) node[below, black, pos=0] {$x_0$};
          \draw[dashed, gray] (0,1.3) -- (2,1.3) node[left, black, pos=0] {$y_0$};
          
          % Tangent line: slope f'(2) = 0.4*2 = 0.8
          \draw[blue, thick] (1, 0.5) -- (3, 2.1);
          \node[blue, right] at (2.5, 1.5) {$f^\prime(x_0)$};
        \end{scope}
        
        % --- Right plot: Inverse f^{-1} ---
        \begin{scope}[xshift=6cm]
          \draw[->] (-0.5,0) -- (3,0) node[right] {$E_2$};
          \draw[->] (0,-0.5) -- (0,3) node[above] {$E_1$};
          
          % Curve for f^{-1} (reflection of f)
          \draw[thick, domain=0.5:2.5, samples=100] plot (\x, {sqrt((\x-0.5)/0.2)});
          \node at (1.2,2.5) {$f^{-1}$};
          
          % Point y0 = 1.3, x0 = 2
          \coordinate (P2) at (1.3, 2);
          \fill (P2) circle (0.06);
          \draw[dashed, gray] (1.3,0) -- (1.3,2) node[below, black, pos=0] {$y_0$};
          \draw[dashed, gray] (0,2) -- (1.3,2) node[left, black, pos=0] {$x_0$};
          
          % Tangent line: reciprocal slope 1/0.8 = 1.25
          \draw[blue, thick] (0.5, 1) -- (2.1, 3);
          \node[blue, left] at (4, 1.5) {$(f^{-1})^\prime(y_0) = \frac{1}{f^\prime(x_0)}$};
        \end{scope}
      \end{tikzpicture}
      \caption{Geometric relationship between the derivative of $f$ at $x_0$ and $f^{-1}$ at $y_0$.}
    \end{figure}
  \end{theorem}
  \begin{proof}
    Let $y_0 = f(x_0)$. Since $f^{-1}$ is continuous at $y_0$, for any sequence $y \to y_0$ in $E_2 \setminus \{y_0\}$, the sequence $x = f^{-1}(y)$ must converge to $x_0 = f^{-1}(y_0)$ in $E_1$. Because $f$ is injective, $y \neq y_0 \implies x \neq x_0$.

    The difference quotient for $f^{-1}$ at $y_0$ is:
    \begin{equation}
      \frac{f^{-1}(y) - f^{-1}(y_0)}{y - y_0} = \frac{x - x_0}{f(x) - f(x_0)}
    \end{equation}

    Since $f$ is differentiable at $x_0$ and $f^\prime(x_0) \neq 0$, we apply the limit:
    \begin{equation}
      \lim_{y \to y_0} \frac{f^{-1}(y) - f^{-1}(y_0)}{y - y_0} = \lim_{x \to x_0} \frac{1}{\frac{f(x) - f(x_0)}{x - x_0}} = \frac{1}{f^\prime(x_0)}
    \end{equation}

    This shows that $f^{-1}$ is differentiable at $y_0$ with $(f^{-1})^\prime(y_0) = 1/f^\prime(x_0)$.
  \end{proof}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1.3]
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Note that if the hypothesis was satisfied, but $f^\prime (x_0) = 0$, then $f^{-1}$ would not be differentiable since it would have an undefined differential. }
  ...
  \begin{theorem}[Differentiation of Exponential and Logarithmic Functions]
    Let $a > 0$, 
    \begin{enumerate}
      \item \textit{Exponential}. The exponential function $x \mapsto a^x$ is differentiable over $\mathbb{R}$, and the derivative is 
        \begin{equation}
          \frac{d}{dx} a^x = a^x \ln(a)
        \end{equation}

      \item \textit{Logarithmic}. The logarithmic function $x \mapsto \log_a x$ is differentiable over $(0, +\infty)$, and the derivative is 
        \begin{equation}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item \textit{Logarithmic}. Since we have defined the logarithm as the inverse of the exponential, by the inverse function theorem, $\ln(x)$ is differentiable. Therefore, by the chain rule, we have 
        \begin{equation}
          1 = \frac{d}{dx} x = \frac{d}{dx} e^{\ln(x)} = e^{\ln(x)} \frac{d}{dx} \ln(x) = x \frac{d}{dx} \ln(x) \implies \frac{d}{dx} \ln(x) = \frac{1}{x}
        \end{equation}
    \end{enumerate}
  \end{proof}

  \begin{theorem}[Power Rule of Differentiation for Reals][thm:power]
    For $\alpha \in \mathbb{R}$, the power function $x \mapsto x^\alpha$ is differentiable over the \textit{interior} of its domain\footnote{Depending on $\alpha$ and $x$, the domain may be different. Consider $x^2$ which has domain $\mathbb{R}$ versus $\sqrt{x}$ which has domain $[0, +\infty)$ and has derivative defined over $(0, +\infty)$. More generally, when $\alpha$ is irrational or is a rational with even denominator, the $x^\alpha$ is not even well-defined for $x < 0$.} and the derivative is 
    \begin{equation}
      \frac{d}{dx} x^\alpha = \alpha x^{\alpha - 1} 
    \end{equation}
  \end{theorem}
  ...
  \begin{theorem}[Differentiation of Trigonometric Functions]
    ...
    \begin{enumerate}
      \item \textit{Sine}. The sine function $x \mapsto \sin(x)$ is differentiable over $\mathbb{R}$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \sin(x)$}, axis lines=middle, samples=100, domain=-pi:pi, xtick={-pi, 0, pi}, xticklabels={$-\pi$, $0$, $\pi$}]
              \addplot[blue, thick] {sin(deg(x))};
            \end{axis}
            \begin{axis}[xshift=0.5\textwidth, width=0.45\textwidth, title={$f'(x) = \cos(x)$}, axis lines=middle, samples=100, domain=-pi:pi, xtick={-pi, 0, pi}, xticklabels={$-\pi$, $0$, $\pi$}]
              \addplot[red, thick] {cos(deg(x))};
            \end{axis}
          \end{tikzpicture}
          \caption{Sine and its derivative} 
          \label{fig:sine_deriv}
        \end{figure}
        

      \item \textit{Cosine}. The cosine function $x \mapsto \cos(x)$ is differentiable over $\mathbb{R}$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \cos(x)$}, axis lines=middle, samples=100, domain=-pi:pi, xtick={-pi, 0, pi}, xticklabels={$-\pi$, $0$, $\pi$}]
              \addplot[blue, thick] {cos(deg(x))};
            \end{axis}
            \begin{axis}[xshift=0.5\textwidth, width=0.45\textwidth, title={$f'(x) = -\sin(x)$}, axis lines=middle, samples=100, domain=-pi:pi, xtick={-pi, 0, pi}, xticklabels={$-\pi$, $0$, $\pi$}]
              \addplot[red, thick] {-sin(deg(x))};
            \end{axis}
          \end{tikzpicture}
          \caption{Cosine and its derivative} 
          \label{fig:cosine_deriv}
        \end{figure}
        

      \item \textit{Tangent}. The tangent function $x \mapsto \tan(x)$ is differentiable over $\mathbb{R} \setminus \{\frac{(2z+1)}{2} \pi\}_{z \in \mathbb{Z}}$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \tan(x)$}, axis lines=middle, samples=200, domain=-2:2, restrict y to domain=-10:10]
              \addplot[blue, thick] {tan(deg(x))};
            \end{axis}
            \begin{axis}[xshift=0.5\textwidth, width=0.45\textwidth, title={$f'(x) = \sec^2(x)$}, axis lines=middle, samples=200, domain=-2:2, restrict y to domain=0:20]
              \addplot[red, thick] {1/(cos(deg(x))^2)};
            \end{axis}
          \end{tikzpicture}
          \caption{Tangent and its derivative} 
          \label{fig:tan_deriv}
        \end{figure}
        

      \item \textit{Arcsine}. The inverse sine function $x \mapsto \arcsin(x)$ is differentiable on $(-1, 1)$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \arcsin(x)$}, axis lines=middle, samples=100, domain=-1:1]
              \addplot[blue, thick] {asin(x)};
            \end{axis}
            \begin{axis}[xshift=0.5\textwidth, width=0.45\textwidth, title={$f'(x) = \frac{1}{\sqrt{1-x^2}}$}, axis lines=middle, samples=200, domain=-0.95:0.95]
              \addplot[red, thick] {1/sqrt(1-x^2)};
            \end{axis}
          \end{tikzpicture}
          \caption{Arcsine and its derivative} 
          \label{fig:asin_deriv}
        \end{figure}
        

      \item \textit{Arccosine}. The inverse cosine function $x \mapsto \arccos(x)$ is differentiable on $(-1, 1)$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \arccos(x)$}, axis lines=middle, samples=100, domain=-1:1]
            ...
            \end{axis}
          \end{tikzpicture}
          \caption{Arccosine and its derivative} 
          \label{fig:acos_deriv}
        \end{figure}
        

      \item \textit{Arctangent}. The inverse tangent function $x \mapsto \arctan(x)$ is differentiable over $\mathbb{R}$, and the derivative is 
        \begin{equation}
        ...
        \begin{figure}[H]
          \centering 
          \begin{tikzpicture}
            \begin{axis}[width=0.45\textwidth, title={$f(x) = \arctan(x)$}, axis lines=middle, samples=100, domain=-5:5]
              \addplot[blue, thick] {atan(x)};
            \end{axis}
            \begin{axis}[xshift=0.5\textwidth, width=0.45\textwidth, title={$f'(x) = \frac{1}{1+x^2}$}, axis lines=middle, samples=100, domain=-5:5]
              \addplot[red, thick] {1/(1+x^2)};
            \end{axis}
          \end{tikzpicture}
          \caption{Arctangent and its derivative} 
          \label{fig:atan_deriv}
        \end{figure}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item \textit{Tangent}. We simply use the quotient rule. 
        \begin{equation}
          \frac{d}{dx} \tan(x) = \frac{d}{dx} \frac{\sin(x)}{\cos(x)} = \frac{\cos^2 (x) + \sin^2(x)}{\cos^2(x)} = \frac{1}{\cos^2(x)} = \sec^2 (x)
        \end{equation}

      \item \textit{Arcsine}. Since the arcsine is the inverse of the sine function, by the inverse function theorem, $\arcsin$ is differentiable. So, using the chain rule, we have 
        \begin{equation}
          1 = \frac{d}{dx} x = \frac{d}{dx} \sin(\arcsin(x)) = \cos(\arcsin(x)) \, \frac{d}{dx} \arcsin(x) 
        \end{equation}
      ...
      \item \textit{Arccosine}. Since arccosine is the inverse of the cosine function, by the inverse function theorem, $\arccos$ is differentiable. So, using the chain rule, we have 
        \begin{equation}
          1 = \frac{d}{dx} x = \frac{d}{dx} \cos(\arccos(x)) = -\sin(\arccos(x)) \, \frac{d}{dx} \arccos(x) 
        \end{equation}
        But since $\sin(x) = \sqrt{1 - \cos^2 (x)}$, we have 
        \begin{equation}
          \frac{d}{dx} \arccos(x) = - \frac{1}{\sin(\arccos(x))} = - \frac{1}{\sqrt{1 - \cos^2(\arccos(x))}} = -\frac{1}{\sqrt{1 - x^2}}
        \end{equation}

      \item \textit{Arctangent}. Since arctangent is the inverse of the tangent function, by the inverse function theorem, $\arctan$ is differentiable. So, using the chain rule, we have 
        \begin{equation}
          1 = \frac{d}{dx} x = \frac{d}{dx} \tan(\arctan(x)) = \sec^2 (\arctan(x)) \, \frac{d}{dx} \arctan(x) 
        \end{equation}
        But since $1 + \tan^2 (x) = \sec^2 (x)$, we have 
        \begin{equation}
          \frac{d}{dx} \arccos(x) = \frac{1}{\sec^2(\arctan(x))} = \frac{1}{1 + \tan^2 (\arctan(x))} = \frac{1}{1 + x^2}
        \end{equation}
    \end{enumerate}
  \end{proof}
  ...
  \begin{example}
    ...
    \begin{equation}
      f: \mathbb{R} \to \mathbb{R}, \quad f(x) \coloneqq \begin{cases} 
        x^2 & \text{ if } x \geq 0 \\ 
        x^3 & \text{ if } x < 0
      \end{cases}
    \end{equation}
    ...
    \begin{align}
      f^\prime_{-} (0) & = \lim_{h \to 0^-} \frac{f(h) - f(0)}{h} = \lim_{h \to 0^-} \frac{h^3}{h} = \lim_{h \to 0^-} h^2 = 0 \\ 
      f^\prime_{+} (0) & = \lim_{h \to 0^+} \frac{f(h) - f(0)}{h} = \lim_{h \to 0^-} \frac{h^2}{h} = \lim_{h \to 0^+} h = 0 
    \end{align}
    Since the \hyperref[thm:two-one-sided]{right and left hand limits coincide, the limit exists} and thus by definition $f^\prime(0) = 0$. 
    ...
    \begin{equation}
      g^\prime (0) = 0, \quad h^\prime (0) = 0
    \end{equation}
  \end{example}

  \begin{example}[Differentiability at Endpoints][ex:wiggly-sine]
    Consider the function 
    \begin{equation}
      f: [0, 1] \to \mathbb{R}, \quad f(x) \coloneqq \begin{cases} 
        x^2 \sin (1/x^2) & \text{ if } x \in (0, 1] \\
        0 & \text{ if } x = 0
      \end{cases}
    \end{equation}
    On $(0, 1]$, $f$ as the composition and product of two differentiable functions is differentiable, and so we can compute $f^\prime$ over $(0, 1]$. However, $f(0) = 0$, so we can't use product rule. We must just use the limit definition. 
    \begin{equation}
      f^\prime (0) = \lim_{h \to 0} \frac{f(h) - f(0)}{h} = \lim_{h \to 0^+} \frac{h^2 \sin(1/h^2)}{h} = \lim_{h \to 0^+} h \sin(1/h^2) = 0
    \end{equation}
    Therefore, we solved 
    \begin{equation}
      f^\prime : [0, 1] \to \mathbb{R}, \quad f^\prime (x) \coloneqq \begin{cases} 
        2 x \sin(1/x) - \frac{2}{x} \cos \frac{1}{x^2} & \text{ if } x \in (0, 1] \\ 
        0 & \text{ if } x = 0
      \end{cases}
    \end{equation}
    ...
    \begin{center}
    \begin{tikzpicture}
      % Shared axis settings for both plots
      \pgfplotsset{
        my axis style/.style={
      ...
      \begin{scope}
        \begin{axis}[
          ...
          \addplot [blue, thick] {x^2 * sin(1/x^2)};
          \addplot [dashed, gray] {x^2};
          \addplot [dashed, gray] {-x^2};
        \end{axis}
      \end{scope}

      % Right Plot: The Derivative f'(x)
      \begin{scope}[xshift=7.5cm]
        \begin{axis}[
          ...
          \addplot [red, thick] {2*x*sin(1/x^2) - (2/x)*cos(1/x^2)};
        \end{axis}
      \end{scope}
    \end{tikzpicture}
    \end{center}
...
\subsection{Higher Order Derivatives and Continuously Differentiablity}
  ...
  \begin{definition}[$C^k$ Functions][def:ck-func]
  ...
  \begin{theorem}[L'Hopital's Rule][thm:lhopital]
  ...
  \begin{proof}
    ...
    \begin{align}
      \lim_{x \to c} \frac{f(x)}{g(x)} 
			...
			& = \frac{\lim_{x \to c} f^\prime (x)}{\lim_{x \to c} g^\prime (x)} \\ 
...
\subsection{Extrema and Concavity}
  ...
  \begin{theorem}[First Derivative Test][thm:first-der-test]
  ...
  \begin{lemma}[Jensen's Inequality][thm:jensen]
  ...
  \begin{theorem}[Second Derivative Test][thm:second-der-test]
...
\subsection{Mean Value Theorem}
  ...
  \begin{theorem}[Rolle's Theorem][thm:rolle]
    Suppose $f: [a, b] \to \mathbb{R}$ is differentiable on $(a, b)$. Then, if $f(a) = f(b)$, then there exists a $c \in (a, b)$ such that $f^\prime (c) = 0$. 
  \end{theorem}
  \begin{proof}
		Since $f$ is continuous on $[a, b]$, by the \hyperref[pst-thm:evt]{extreme value theorem}, it has to attain its global max and min values somewhere in $[a, b]$. If either is in $(a, b)$, then \hyperref[thm:first-der-test]{the derivative is $0$}. If max and min are attained on $\{a, b\}$, then since $f(a) = f(b)$, this implies that $f(x) = f(a)$ for all $x \in [a, b]$, which implies $f^\prime (x) = 0$. 
  \end{proof}

  \begin{theorem}[Mean Value Theorem][thm:mvt]
...
\subsection{Monotone Functions} 
	...
	\begin{definition}[Monotone]
		A function $f: E \subset \mathbb{R} \to \mathbb{R}$ is 
		\begin{enumerate}
			\item \textbf{monotonically increasing} if $x < y \implies f(x) \leq f(y)$. 
			\item \textbf{monotonically decreasing} if $x < y \implies f(x) \geq f(y)$. 
		  \item \textbf{strictly monotonically increasing} if $x < y \implies f(x) < f(y)$. 
		  \item \textbf{strictly monotonically increasing} if $x < y \implies f(x) > f(y)$. 
		\end{enumerate}
		for all $x, y \in E$. We say $f$ is (strictly) monotone if is is (strictly) monotonically increasing or decreasing. 
	\end{definition}

	Often, we restrict the domain of a monotone function to be an interval. 

	\begin{definition}[Interval]
		An \textbf{interval} $I \subset \mathbb{R}$ is a set of the form $[a, b], [a, b), (a, b], (a, b)$. 
	\end{definition}

	\begin{theorem}[Continuous Injective Function is Monotone]
		\label{thm:cont_inj_monotone}
		Let $f: I \to \mathbb{R}$ be continuous. If $f$ is injective, then $f$ is strictly monotone. 
	\end{theorem}
	\begin{proof}
		We prove the contrapositive. If $f$ is not strictly monotone, then there exists points $x, y, z \in I$, with $x < y < z$, such that $f(x) \geq f(y) \leq f(z)$ or $f(x) \leq f(y) \geq f(z)$. Let's focus on the first case: $f(x) \geq f(y) \leq f(z)$. If any of them are equal, then $f$ is not injective, so $f(x) > f(y) < f(z)$. There are two cases. 
		\begin{enumerate}
		  \item $f(x) > f(z)$. Then, we have $f(y) < f(z) < f(x)$, and by the intermediate value theorem, there exists a $c \in (x, y)$ such that $f(c) = f(z)$, which implies $f$ is not injective. 
			\item $f(x) < f(z)$. Then, we have $f(y) < f(x) < f(z)$, and by the intermediate value theorem, there exists a $c \in (y, z)$ such that $f(c) = f(x)$, which implies $f$ is not injective. 
		\end{enumerate}
		We apply the same logic to the second case, and we are done. 
	\end{proof}

  \begin{theorem}[Derivative of a Monotonic Function]
		\label{thm:der_mono_func}
		Given function $f: [a, b] \longrightarrow \mathbb{R}$ that is differentiable on $(a, b)$,\footnote{Note the one-sided direction for the strict inequalities. }
    \begin{enumerate}
      \item $f^\prime > 0 \implies f$ is strictly increasing
      \item $f^\prime \geq 0 \iff f$ is increasing
      \item $f^\prime = 0 \iff f$ is constant
      \item $f^\prime \leq 0 \iff f$ is decreasing
      \item $f^\prime < 0 \implies f$ is strictly decreasing
    \end{enumerate}
		The reverse implication is a bit weaker. 
    \begin{enumerate}
      \item $f \text{ is strictly increasing} \implies f^\prime (x) \geq 0$
      \item $f \text{ is strictly decreasing} \implies f^\prime (x) \leq 0$
    \end{enumerate}
  \end{theorem}
  \begin{proof}
    ...
    \begin{equation}
      0 = f(x_2) - f(x_1) = f^\prime (c) (x_2 - x_1)
    \end{equation}
    implies that $f^\prime (c) = 0$ for all $x_1, x_2 \in E$. 
  \end{proof}
	\begin{proof}
		Here is another proof, where we prove the contrapositive. Let there be an $x \in E$ s.t. $f^\prime (x) < 0$. Then, 
		\begin{equation}
		...
		\begin{equation}
			|x - y| < \delta \implies \frac{f(y) - f(x)}{y - x} < 0
		\end{equation}
	...
	Note that $f$ strictly monotone does not imply that $f^\prime$ is strictly greater than or less than $0$! 

	\begin{example}
	  Let $f(x) = x^3$. $f$ is strictly increasing on $(-1, 1)$, but $f^\prime (0) = 0$. 
	\end{example}

	\begin{corollary}[Equal Derivatives Imply Constant Difference]
		Given two functions $f, g: E \subset \mathbb{R} \to \mathbb{R}$ have the same derivatives---i.e. $f^\prime = g^\prime$---then the difference is a constant
		\begin{equation}
		  f - g = c
		\end{equation}
  \end{corollary}
  \begin{proof}
		Since $f^\prime - g^\prime = (f - g)^\prime = 0$, it must be the case that $f - g$ is constant. 
  \end{proof}

	We end this with a pathological example of a monotonically increasing function. 
  ...
  \begin{theorem}[Properties of Cantor-Lebesgue Function]
    $\phi$ is a nondecreasing, continuous function s.t. $\phi^\prime (x) = 0$ for all $x \in O$. 
  \end{theorem}
  \begin{proof}
    Listed. 
    \begin{enumerate}
...
\subsection{Taylor's Formula} 
  ...
  \begin{theorem}[Taylor's Theorem with Peano's Form of Remainder][thm:taylor-peano]
  ...
  \begin{theorem}[Taylor's Theorem with Lagrange's Form of Remainder][thm:taylor-lagrange]
    Suppose $f: [a, b] \to \mathbb{R}$
    \begin{enumerate}

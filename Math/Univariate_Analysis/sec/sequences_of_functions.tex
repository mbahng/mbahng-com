\section{Sequences of Functions}
  ...
  As daunting as the behavior of sequences of functions can be for the first-timer, most of the theory of sequences of real numbers is pretty much solved. In real applications, we are primarily interested not in the convergence of \textit{functions}. For example, in differential equations, we are solving for some function, and we may approximate the solution with a sequence of simpler functions. In probability theory, random variables---as we will see later---are simply functions, and studying any stochastic process (e.g. Markov chains), is the same as studying sequences of functions. Therefore, let's first establish what it means for a function to converge. 

  \begin{definition}[Pointwise Convergence][def:pointwise-convergence]
    Let $E \subset \mathbb{R}$ and $f, f_n : E \to \mathbb{R}$. We say that $f_n$ \textbf{converges pointwise} to $f$, denoted $f_n \to f$, if 
    \begin{equation}
...
\subsection{Modes of Escape}
  ...
  \end{equation} 
  The answer is no, and there are a few general ways it may fail to do so.\footnote{Thanks to \href{https://terrytao.wordpress.com/2010/10/02/245a-notes-4-modes-of-convergence/}{terrytao.wordpress.com/2010/10/02/245a-notes-4-modes-of-convergence/}.}

  \begin{example}[Escape to Horizontal Infinity]
    The following function converges to $0$ pointwise. 
    \begin{equation}
      f_n: (0, +\infty) \to \mathbb{R}, \quad f_n \coloneqq \mathbbm{1}_{[n, n+1]} 
    \end{equation}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=0.7]
        ...
        % f_1: [1,2] height 1
        \draw[blue, thick] (1,0) -- (1,1) -- (2,1) -- (2,0);
        \node[blue, above] at (1.5,1) {\footnotesize $f_1$};
        
        % f_2: [2,3] height 1
        \draw[red, thick] (2,0) -- (2,1) -- (3,1) -- (3,0);
        \node[red, above] at (2.5,1) {\footnotesize $f_2$};
        
        % f_3: [3,4] height 1
        \draw[orange, thick] (3,0) -- (3,1) -- (4,1) -- (4,0);
        \node[orange, above] at (3.5,1) {\footnotesize $f_3$};
  ...
  \begin{example}[Escape to Width Infinity]
    The following function converges to $0$ pointwise. 
    \begin{equation}
      f_n: (0, +\infty) \to \mathbb{R}, \quad f_n \coloneqq \frac{1}{n} \mathbbm{1}_{[0, n]} 
    \end{equation}
    However, note that $\int f_n = 1$, but $\int f = 0$. 
    ...
    \begin{center}
      \begin{tikzpicture}[scale=0.7]
        ...
        \node[blue, left] at (0,1) {\footnotesize $1$};
        
        % f_2: [0,2] height 0.5
        \draw[red, thick] (0,0) -- (0,0.5) -- (2,0.5) -- (2,0);
        \node[red, left] at (0,0.5) {\footnotesize $1/2$};
        
        % f_4: [0,4] height 0.25
        \draw[orange, thick] (0,0) -- (0,0.25) -- (4,0.25) -- (4,0);
        \node[orange, left] at (0,0.25) {\footnotesize $1/4$};
  ...
  \begin{example}[Escape to Vertical Infinity]
    The following function converges to $0$ pointwise. 
    \begin{equation}
      f_n: (0, 1] \to \mathbb{R}, \quad f_n \coloneqq n \mathbbm{1}_{[\frac{1}{n}, \frac{2}{n}]} 
    \end{equation}
    However, note that $\int f_n = 1$, but $\int f = 0$. 
    ...
    \begin{center}
      \begin{tikzpicture}[scale=0.7]
        ...
        % f_1: [1,2] height 1
        \draw[blue, thick] (1,0) -- (1,1) -- (2,1) -- (2,0);
        \node[blue, right] at (2,0.5) {\footnotesize $f_1$};
        
        % f_2: [0.5,1] height 2
        \draw[red, thick] (0.5,0) -- (0.5,2) -- (1,2) -- (1,0);
        \node[red, right] at (1,1.5) {\footnotesize $f_2$};
        
        % f_4: [0.25,0.5] height 4
        \draw[orange, thick] (0.25,0) -- (0.25,4) -- (0.5,4) -- (0.5,0);
        \node[orange, right] at (0.5,3.5) {\footnotesize $f_4$};
...
\subsection{Conditions for Uniform Convergence}
  ...
  \begin{definition}[Uniform Convergence][def:uniform-convergence]
...
\subsection{Consequences of Uniform Convergence} 
...
\subsection{Exercises} 
  ...
  \begin{exercise}[Rudin 7.5]
    Let
    \begin{equation}
      f_n(x) = 
      \begin{cases} 
        0 & (x < \frac{1}{n+1}) \\
        \sin^2 \frac{\pi}{x} & (\frac{1}{n+1} \le x \le \frac{1}{n}) \\
        0 & (\frac{1}{n} < x)
      \end{cases}
    \end{equation}
  ...
  \begin{exercise}[Rudin 7.11]
    Suppose $\{f_n\}$, $\{g_n\}$ are defined on $E$, and
    \begin{enumerate}
      \item[(a)] $\sum f_n$ has uniformly bounded partial sums;
      \item[(b)] $g_n \to 0$ uniformly on $E$;
      \item[(c)] $g_1(x) \ge g_2(x) \ge g_3(x) \ge \dots$ for every $x \in E$.
    \end{enumerate}
    Prove that $\sum f_n g_n$ converges uniformly on $E$. \textit{Hint:} Compare with Theorem 3.42.
  \end{exercise}
  \begin{solution}

  \end{solution}

  \begin{exercise}[Rudin 7.12]
  ...
  \begin{exercise}[Rudin 7.13]
    Assume that $\{f_n\}$ is a sequence of monotonically increasing functions on $R^1$ with $0 \le f_n(x) \le 1$ for all $x$ and all $n$.
    \begin{enumerate}
      \item[(a)] Prove that there is a function $f$ and a sequence $\{n_k\}$ such that
        \begin{equation}
      ...
      \item[(b)] If, moreover, $f$ is continuous, prove that $f_{n_k} \to f$ uniformly on compact sets.
    \end{enumerate}
  \end{exercise}
  \begin{solution}
  ...
  \begin{exercise}[Rudin 7.14]
    Let $f$ be a continuous real function on $R^1$ with the following properties: $0 \le f(t) \le 1, f(t + 2) = f(t)$ for every $t$, and
    \begin{equation}
      f(t) = 
      \begin{cases}
        0 & (0 \le t \le \frac{1}{3}) \\
  ...
  \begin{exercise}[Rudin 7.19]
    Let $K$ be a compact metric space, let $S$ be a subset of $\mathscr{C}(K)$. Prove that $S$ is compact (with respect to the metric defined in Section 7.14) if and only if $S$ is uniformly closed, pointwise bounded, and equicontinuous.
  \end{exercise}
  \begin{solution}

  \end{solution}

  \begin{exercise}[Rudin 7.20]
  ...
  \begin{exercise}[Rudin 7.24]
    ...
    \end{equation}
    Prove that $|f_p(x)| \le d(a, p)$ for all $x \in X$, and that therefore $f_p \in \mathscr{C}(X)$. Prove that $\|f_p - f_q\| = d(p, q)$.
  \end{exercise}
  \begin{solution}

  \end{solution}

  \begin{exercise}[Rudin 7.25]
    Suppose $\phi$ is a continuous bounded real function in the strip defined by $0 \le x \le 1, -\infty < y < \infty$. Prove that the initial-value problem
    \begin{equation}
  ...
  \begin{exercise}[Rudin 7.26]
    Prove an analogous existence theorem for the initial-value problem
    \begin{equation}
      \mathbf{y}' = \mathbf{\Phi}(x, \mathbf{y}), \quad \mathbf{y}(0) = \mathbf{c},
    \end{equation}
    where now $\mathbf{c} \in R^k, \mathbf{y} \in R^k$, and $\mathbf{\Phi}$ is a continuous bounded mapping of the part of $R^{k+1}$ defined by $0 \le x \le 1, \mathbf{y} \in R^k$ into $R^k$.
  \end{exercise}
  \begin{solution}

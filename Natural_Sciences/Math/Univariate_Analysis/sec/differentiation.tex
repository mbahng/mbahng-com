\section{Differentiation} 
  ...
  \begin{definition}[Derivative]
    Given $f: [a, b] \subset \mathbb{R} \to \mathbb{R}$, fix $x \in [a, b]$, and define the \textbf{difference quotient} as the following function of $h$.
    \begin{equation}
      \Diff_h (x) = \frac{f(x + h) - f(x)}{h}
    \end{equation}
...
\subsection{Taylor's Formula} 
  ...
  \begin{lemma}[Taylor's Formula of Order 2 with Peano's Form of Remainder]
    ...
    \begin{equation}
      f(x) = f(x_0) + f^\prime (x_0) (x - x_0) + o \big( (x - x_0)\big)
    \end{equation} 
  \end{lemma}
  \begin{proof}
  ...
  \begin{theorem}[Taylor's Theorem with Lagrange's Form of Remainder]
    Suppose $f: [a, b] \to \mathbb{R}$
    \begin{enumerate}
      \item is $n$-times differentiable over $(a, b)$, and 
      \item is $C^{n-1}$ over $[a, b]$. 
    \end{enumerate}
    Then, there exists a $\xi$ between $x_0$ and $x$ s.t. 
    \begin{equation}

\section{Differentiation} 
  ...
  Sometimes, we might want to define a one-sided derivative. Most of the times, this is not needed, but since I've seen it pop up a few times, I'll put it here. 

  \begin{definition}[One-Sided Derivative]
    Let $f: E \subset \mathbb{R} \to \mathbb{R}$. 
    \begin{enumerate}
      \item If $a$ is a limit point of $E \cap [a, +\infty)$, then the following limit---if it exists---is called the \textbf{right derivative of $f$ at $a$}. 
        \begin{equation}
          f^\prime_+ (a) = \lim_{y \to x^+} \frac{f(y) - f(x)}{y - x}
        \end{equation}

      \item If $a$ is a limit point of $E \cap (-\infty, a]$, then the following limit---if it exists---is called the \textbf{left derivative of $f$ at $a$}. 
        \begin{equation}
          f^\prime_- (a) = \lim_{y \to x^-} \frac{f(y) - f(x)}{y - x}
        \end{equation}
    \end{enumerate}
...
\subsection{Rules of Differentiation}
  ...
  \begin{theorem}[Power Rule of Differentiation]
  ...
  \begin{theorem}[Differentiation of Exponential and Logarithmic Functions]
    We have for all $a > 0$, 
    \begin{equation}
      \frac{d}{dx} a^x = a^x \log(a), \qquad \frac{d}{dx} \log_a (x) = \frac{1}{x \ln(a)} 
    \end{equation}
    And in particular, 
    \begin{equation}

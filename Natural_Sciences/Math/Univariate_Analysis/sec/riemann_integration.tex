\section{Riemann Integration}
  ...
  Now we will construct the theory of integration, which can be informally thought of as an summation for uncountable sets. Note that for now, this is a completely separate construction from the derivative, and can even be established independently from it. The unification of the two comes from the fundamental theorem of calculus. 

\subsection{Darboux Integration}

  We would like to define an integral. If you have taken high school calculus, we can do this by defining the Riemann sums for a particular partition, which is some number in $\mathbb{R}$. By taking lower and upper approximations and then taking the supremum and infimum of them, respectively, we can determine what it means for a function to be Riemann integrable. 
  ...
  \begin{definition}[Darboux Integral]
    ...
    \end{equation}
    If the upper and lower Riemann integrals are equal, then $f$ is said to be \textbf{Darboux integrable} over $[a, b]$, denoted $f \in \mathcal{R}([a, b])$.\footnote{Where $\mathcal{R}(X)$ is the set of all Riemann integrable functions over $X$.}
  \end{definition} 
...
\subsubsection{Riemann Integration}
  ...
  Historically, a slightly earlier form of the integral invented by Riemann is the Riemann integral. In here, he uses a tagged partition. 

  \begin{definition}[Tagged Partition]
    A \textbf{tagged partition}, or a \textbf{partition with distinguished points}, is a partition $P$ along with some set of $\xi_i$'s that land in each interval. 
  \end{definition}
  ...
  \begin{theorem}[Equivalence of Riemann and Darboux Integrables]
...
\subsection{Riemann-Stieltjes Integral}
  ...
  \begin{definition}[Riemann-Stieltjes Integral]
    The \textbf{Riemann-Stieltjes integral}. 
  \end{definition}

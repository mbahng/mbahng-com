...
\subsection{Darboux Integration}
  ...
  \begin{definition}[Darboux Integral][def:riemann-integral]
    Let $f: [a, b] \to \mathbb{R}$ be bounded. Then, the \textbf{upper and lower Darboux integrals} of $f(x)$ are defined to be the real numbers
    \begin{equation}
  ...
  \begin{example}
    The function 
    \begin{equation}
      f: [0, 1] \to \mathbb{R}, \quad f(x) = \begin{cases} 
        \frac{1}{x} & \text{ if } x \in (0, 1] \\ 
        \alpha & \text{ if } x = 0 
      \end{cases}
    \end{equation}
    is not Riemann integrable over $[0, 1]$ for all values of $\alpha$ since $f$ is unbounded around $0$. 
  \end{example}
...
\subsection{Conditions for Riemann Integrability}
  ...
  Intuitively, a function $f$ is Riemann integrable if we can make $U(f, P) - L(f, P)$ as small as we wish. This is the case if we can find a sufficiently refined partition $P$ such that the \hyperref[def:oscillation]{oscillation} on $f$ on each interval is small. 
  ...
  \begin{theorem}[Triangle Inequality for Riemann Integrals][thm:triangle-int]
    We have
    \begin{equation}
...
\subsection{The Fundamental Theorem of Calculus} 
  ...
  \begin{example}[Cannot Apply Fundamental Theorem for Nonintegrable Functions]
    A common mistake that one can make is to apply the fundamental theorem for non-Riemann integrable functions. For example, one may consider evaluating 
    \begin{equation}
      \int_{-1}^1 x^{-2} = \bigg(-\frac{1}{x}\bigg) \bigg|_{-1}^1 = -1 - 1 = -2
    \end{equation}
...
\subsection{Improper Integrals}
  ...
  \begin{example}
    ...
    \end{equation}
    \begin{enumerate}
      \item It is not Riemann integrable over $(0, 1]$ since it is unbounded around $0$. However, it is defined for all $[\epsilon, 1]$. So, we can define the improper integral
        \begin{equation}
          \int_0^1 x^{-1/2} \coloneqq \lim_{\epsilon \to 0^+} \int_\epsilon^1 x^{1/2} = \lim_{\epsilon \to 0^+} (2 x^{1/2}) \big|_\epsilon^1 = \lim_{\epsilon \to 0^+} \big( 2 - 2\sqrt{\epsilon}) = 2
        \end{equation}
      \item It is not Riemann integrable over $[1, +\infty)$ since the domain of integration is not bounded. However, it is defined for all $[1, h]$. So we can define the improper integral 
        \begin{equation}
          \int_1^{+\infty} x^{-1/2} \coloneqq \lim_{h \to +\infty} \int_1^h x^{1/2} = \lim_{h \to +\infty} (2 x^{1/2}) \big|_1^{h} = +\infty
        \end{equation}
        So the improper Riemann integral diverges. 
    \end{enumerate}
  \end{example}

  \begin{example}[Vertical Asymptotes]
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          % Shading the area (optional, to represent the integral)
          \addplot [fill=blue, fill opacity=0.1, draw=none, domain=-1:-0.1] {1/x^2} \closedcycle;
          \addplot [fill=blue, fill opacity=0.1, draw=none, domain=0.1:1] {1/x^2} \closedcycle;
        \end{axis}
      \end{tikzpicture}
      \caption{Plot of $f(x) = \frac{1}{x^2}$ showing the vertical asymptote at $x=0$.} 
      \label{fig:inverse_square_plot}
    \end{figure}
    ...
    \begin{align}
      \int_{-1}^1 x^{-2} & = \int_{-1}^0 x^{-2} + \int_0^1 x^{-2} \\  
                         & = \lim_{\epsilon \to 0^-} \int_{-1}^\epsilon x^{-2} + \lim_{\epsilon \to 0^+} \int_\epsilon^{1} x^{-2} \\ 
                         & = \lim_{\epsilon \to 0^-} \bigg( -\frac{1}{x} \bigg) \bigg|_{-1}^\epsilon + \lim_{\epsilon \to 0^+} \bigg( -\frac{1}{x} \bigg) \bigg|_\epsilon^1 \\ 
                         & = -2 + \lim_{\epsilon \to 0^-} -\frac{1}{\epsilon} + \lim_{\epsilon \to 0^+} \frac{1}{\epsilon} \\ 
                         & = -2 + \infty + \infty = +\infty
    \end{align}
  \end{example}

  \begin{example}[Essential Discontinuities]
    Consider the function and its derivative $f, f^\prime : [0, 1] \to \mathbb{R}$ which we have derived \hyperref[ex:wiggly-sine]{here}. 
    \begin{equation}
      f(x) \coloneqq \begin{cases} 
        x^2 \sin (1/x^2) & \text{ if } x \in (0, 1] \\
        0 & \text{ if } x = 0
      \end{cases}, \quad f^\prime (x) \coloneqq \begin{cases} 
        2 x \sin(1/x) - \frac{2}{x} \cos \frac{1}{x^2} & \text{ if } x \in (0, 1] \\ 
        0 & \text{ if } x = 0
      \end{cases}
    \end{equation}
    Therefore, $f$ is a primitive of $f^\prime$. However, $f^\prime$ is not Riemann integrable since it is unbounded around $0$. But for every $\epsilon > 0$, $f^\prime$ is continuous and bounded---and hence Riemann integrable---over $[\epsilon, 1]$, so now we can apply FTC. 
    \begin{equation}
      \int_\epsilon^1 f^\prime = f(1) - f(\epsilon)
    \end{equation}
    By taking the limit as $\epsilon \to 0^+$ on both sides, we get 
    \begin{equation}
      \int_{0}^1 f^\prime \coloneqq \lim_{\epsilon \to 0^+} f^\prime = f(1) - \lim_{\epsilon \to 0^+} f(\epsilon) = f(1) - f(0)
    \end{equation}
  ...
  \begin{example}[Sinc Function][ex:sinc]
    Consider the integral 
    \begin{equation}
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          domain = -19:19,
          width = 12cm, height = 5cm, % Wide aspect ratio
          ...
          % Plot sin(x)/x.
          % Using a conditional to handle the removable singularity at x=0.
          % Note: pgfplots' sin() function expects degrees, so we use deg(x).
          \addplot [
            blue,
            thick,
            mark=none
        ...
        \end{axis}
      \end{tikzpicture}
      \caption{Plot of the function $f(x) = \frac{\sin(x)}{x}$. The integral is improper due to the unbounded domain $(-\infty, +\infty)$, despite the function itself being bounded.} 
      \label{fig:sinc_integral}
    \end{figure}

    We first show that $\int_0^{+\infty} \frac{\sin(x)}{x} \, dx$ converges. By the symmetry of the integrand (it is an even function), the total integral will be twice this value. For $h > 1$, we apply integration by parts with $u = \frac{1}{x}$ and $dv = \sin(x) \, dx$:
    
    \begin{align}
      \int_1^h \frac{\sin(x)}{x} \, dx & = \bigg[ -\frac{\cos(x)}{x} \bigg]_1^h - \int_1^h \frac{\cos(x)}{x^2} \, dx \\
                                       & = \cos(1) - \frac{\cos(h)}{h} - \int_1^h \frac{\cos(x)}{x^2} \, dx
    \end{align}

    Taking the limit as $h \to \infty$, the term $\frac{\cos(h)}{h} \to 0$ because $|\cos(h)| \leq 1$. The integral $\int_1^\infty \frac{\cos(x)}{x^2} \, dx$ converges absolutely by the $p$-test, since $\left| \frac{\cos(x)}{x^2} \right| \le \frac{1}{x^2}$ and $\int_1^\infty \frac{1}{x^2} \, dx$ converges. It turns out that 
    \begin{equation}

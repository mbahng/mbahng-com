...
\subsection{Statistical Models} 
  ...
  \begin{example}[Density Estimation Model]
    \begin{enumerate}
      \item $X \sim N(\theta, 1)$ for $\theta \in [-1, 1]$, means the statistical model $(\mathbb{R}, \mathcal{B}(\mathbb{R}), \{N(\theta, 1) \colon \theta \in [-1, 1]\})$. 
      \item $X_1 \ldots X_n \overset{\text{iid}}{\sim} N(\theta, 1)$, $\theta \in \Theta$ is shorthand for the statistical model $(\mathbb{R}^n, \mathcal{B}(\mathbb{R}^n), \{N(\theta, 1)^{\otimes n} \colon \theta \in \Theta\})$. 
    \end{enumerate}
  \end{example}

  \begin{example}[Regression Model][ex:reg-model]
    Say that we have data $\{(x_i, y_i)\}_{i=1}^n$, and we are interested in the \textit{conditional distribution} $Y \mid X = x$. We can treat $x_i$'s as fixed and model the $Y_i$'s as being generated by the following: 
    \begin{equation}
  ...
  \begin{question}
    ...
    The sum of two sigma finite measures is sigma finite. Don't worry about it now, but when you do encounter it in research (which it highly likely), study it then. 
  \end{question}
  ...
  \begin{definition}[Parameter Space]
    A \textbf{parameter space} for the model $\mathcal{P}$ is a set $\Theta$, together with a surjective map $\theta: \mathcal{P} \to \Theta$. 
  \end{definition} 

  Remember that $\theta$ is a \textit{map}, and that since it is not necessarily injective, multiple measures $\mathbf{P}_1, \mathbf{P}_2 \in \mathcal{P}$ can map to the same $\theta$. 

  \begin{definition}[Identifiability]
    A statistical model is \textbf{identifiable} by parameter space $\Theta$ if the map $\theta: \mathcal{P} \to \Theta$ is injective---and hence, bijective. 
  \end{definition} 
  ...
  \begin{definition}
    A \textbf{statistical experiment} is an identifiable statistical model. It is denoted $(\mathcal{X}, \mathscr{X}, \mathcal{P}, \Theta)$. 
  \end{definition}

  If a model is identifiable, we can index the distributions by the parameter value and just write each probability measure as $\mathbf{P}_{\theta}$. Therefore, for identifiable models, we will write as shorthand $\mathcal{P} = \{\mathbf{P}_\theta \colon \theta \in \Theta\}$. 
  ...
  \begin{example}[Linear Redundancy]
    Consider the \href{ex:reg-model}{regression statistical model} again, but now we consider the class of linear functions $\mathcal{F} = \{ f(x) = (\theta_1 + \theta_2)x \colon \theta_1, \theta_2 \in \mathbb{R} \}$, which leads to our model 
    \begin{equation}

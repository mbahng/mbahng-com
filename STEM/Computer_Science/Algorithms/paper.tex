...
\section{Graphs}
  ...
  \subsection{Bipartite Graphs}
    ...
    With this algorithm, if we have a graph with all even cycles, we want to devise an algorithm to actually split this graph into left and right. This is simply BFS starting from any node $s \in V$, which intuitively explores every \textit{layer} going out from $s$ and packs each layer into $L, R$ alternating. 

...
\section{Shared Memory and Concurrency}
  ...
  \subsection{Threads}
    ...
    So far, we've talked about everything as a sequential process of instructions. In practicality, we have improved this from the memory perspective by implementing caches, virtual memory, and swapping, but in the CPU perspective. In CPUs, we can't just simply increase the clock frequency indefinitely since there are physical limitations.\footnote{It turns out that power consumption increases faster than clock frequency, so it scales badly.} The current trend is to increase parallelism to compute faster, which is implemented with cores and threads. Simply, a CPU has multiple cores, each with its own L1/L2 cache, and possibly even a shared L3 cache. There are actually two types of parallelism: at the \textit{process level} and at the \textit{thread level}. 

    \begin{definition}[Context Switch]
      Each core can run a single process, and if there are more processes than cores, then the OS can schedule them by implementing \textbf{context switches} which passes control of executing on a CPU core from one execution context to another. 

      \begin{figure}[H]
        \centering 
        \includegraphics[scale=0.4]{img/context_switch.png}
        \caption{Context switches allow the OS to schedule multiple processes on a single CPU core.Every time context switching happens, the CPU must enter into kernel mode to switch. } 
    ...
    \begin{definition}[Thread and Processes]
      We want to distinguish between the two: 
      \begin{enumerate}
        \item A program simply needs registers (e.g. a \texttt{rsp} stack pointer) and a stack to run, which is known as a \textbf{thread}. 
        \item A thread, plus the virtual address space and a file table, is called a \textbf{process}. 
      \end{enumerate}
      Multiple threads of one process can a therefore share an address spaces. This enables us to divide large tasks across several cooperative threads, and threads can communicate with each other through the shared address space. 
    \end{definition}
    ...
    \begin{theorem}[Implementation in C]
      This is implemented in C with the \texttt{pthread.h} library, which is included in the standard library directory. 
      ...
      \begin{figure}[H]
        ...
        \end{lstlisting}
        \caption{We have a function \texttt{mythread} that we want to run on multiple threads. We first start off by creating a few threads with each function. Since this is running on multiple cores at the same time, it can either print AB or BA. }
        \label{fig:thread_example}
      \end{figure}

      The two main functions needed is \texttt{pthread\_create} and \texttt{pthread\_join}. For \texttt{pthread\_create}, it essentially runs the function on each thread with some specific arguments. It returns $0$ on success, and an error number otherwise. It has the following arguments. 
      \begin{enumerate}
        \item 1: A pointer to a \texttt{pthread\_t} struct
        \item 2: Attributes of the thread, such as stack size (to prevent it from taking too much stack), scheduling priority (\texttt{NULL} for default)
        \item 3: this is the pointer to the start function the thread starts running in. Note that if we declare a function name, then that function name is a pointer to the actual function. 
        \item 4: Pointer to the sole argument to the start function. 
      \end{enumerate}
      Then, for the \texttt{pthread\_join} function, it essentially waits for the thread to finish and returns the return value of the thread. It has the following arguments.
      \begin{enumerate}
        \item 1: A pointer to a \texttt{pthread\_t} struct. 
        \item 2: A pointer to the return value of the thread.
      \end{enumerate}
    ...
    \begin{example}[MWS of Thread Implementation]
      ...
      \begin{lstlisting}
        ...
        int main() {
          ...
          printf("Sum of array is %d computed in %f\n", sum, timing);
          return 0; 
        }
      \end{lstlisting}
      ...
      \begin{lstlisting}
        ...
        void* sum_array(void* arg) {
            ...
            data->partial_sum = 0;
            for (int i = data->start; i < data->start + data->len; i++) {
        ...
        int main() {
            ...
            int segment_length = N / NUM_THREADS;
            ...
            for (int i = 0; i < NUM_THREADS; i++) {
                ...
                data[i].len = (i == NUM_THREADS - 1) ? (N - data[i].start) : segment_length;
            ...
            printf("Sum of array is %d computed in %f seconds.\n", total_sum, timing);

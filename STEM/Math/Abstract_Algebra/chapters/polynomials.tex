\section{Polynomial Rings} 
  ...
  Before we move on, let's get some terms out of the way. 
...
\subsubsection{Roots and Factorization}
  ...
  \begin{theorem}[Root-Factor Theorem]
    Given field $F$,\footnote{In fact, this works for any commutative ring as long as the leading coefficient of the divisor is a unit.} $(x - c)$ is a factor of $f(x)$ if and only if $f(c) = 0$. 
  \end{theorem} 
  \begin{proof}
  ...
  Now, we show an extremely important theorem.  
  ...
  \begin{example}[Linear Polynomial with 3 Roots]
    Consider $f(x) = x^2 - 1 \in \mathbb{Z}_8 [x]$. Then $1, 3, 5, 7$ are all roots of $f(x)$, which is greater than its degree. Furthermore, it has two different factorizations 
    \begin{equation}
...
\subsection{Factorization over Minimal Fields} 

\subsubsection{Fundamental Theorem of Algebra} 

  So far, we have stated things about polynomials over arbitrary fields. Just the field property alone gave us a lot of nice properties such as unique factorization of polynomials. However, we have only proved that the number of roots is \textit{at most} the degree $n$, but not that it actually reaches $n$. It turns out that if we pick a very specific field, i.e. the complex numbers $\mathbb{C}$, we can get a completely linear factorization. Ironically, this theorem cannot be proven with algebra alone. We need complex analysis.\footnote{Gauss proved this for the first time in 1799.} 
...
\subsubsection{Field Extensions}
  ...
  So we must work in \textit{at least} some superfield of $\mathbb{Q}$. A question we can ask is, given a particular polynomial in $\mathbb{Q}$, can we find some smallest possible field $K \subset \mathbb{C}$ containing all its roots? What can we determine about $K$? This notion of taking a field $F$ and creating a sup-field $K \supset F$ will be done many times. 
  ...
  Note that $F \subset F[\alpha]$ since we can just take the constant polynomials, so this is not very interesting. What allows us to make this inclusion proper is that the $\alpha \in K$, which does not necessarily have to be in $F$, \textit{extends} this field a bit further, but since we can only map the one element $\alpha$, it may not cover all of $K$. The most obvious structure is that of a ring. 

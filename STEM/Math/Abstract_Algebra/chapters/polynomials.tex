...
\subsection{Rational Polynomials} 
  ...
  Given a polynomial $f \in \mathbb{Q}[x]$, the fundamental theorem of algebra guarantees that it will have all of its roots in $\mathbb{C}$. Our immediate goal is to find the \textit{smallest} possible field $K \subset \mathbb{C}$ containing them all. What can we determine about $K$? 
  ...
  Therefore, let us formalize this notation. 
  ...
  \begin{definition}[Ring of Univariate Polynomial Elements] 
    ...
    \begin{equation}
      F[\alpha] \coloneqq \{ f(\alpha) \in F \mid f \in F[x]\}
    \end{equation}
    is the subring $F[\alpha] \subset K$. 
  \end{definition} 

  Intuitively, the extra $\alpha \in K$ allows us to ``expand'' our field $F$ into a bigger subring of $K$. We can also define this for multivariate polynomials.  
  ...
  While $F[\alpha]$ guaranteed to be a subring, it may or may not be a field. There is a sufficient condition for it to be a field. 

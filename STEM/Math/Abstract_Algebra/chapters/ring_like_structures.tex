...
\subsection{Divisors} 
  ...
  \begin{definition}[Prime and Compositive Elements]
    Let $R$ be a commutative ring, and let $p \in R$ not be $0$ or a unit. 
    \begin{enumerate}
      \item $p$ is called \textbf{prime} if it has only divisors $1$ and $p$. 
      \item $p$ is called \textbf{composite} if it is not prime. 
    \end{enumerate}
  \end{definition}

...
\section{High Dimensional Linear Regression}
  ...
  \subsection{Ridge Regression} 
    ...
    \begin{theorem}[Least Squares Solution for Ridge Regression]
      ...
      \begin{equation}
        \hat{\beta} = (X^T X+ \lambda I)^{-1} \mathbf{X}^T \mathbf{Y}
      \end{equation}
    \end{theorem}
    \begin{proof}
      TBD
  ...
  \subsection{Forward Stepwise Regression} 
    ...
    \begin{definition}[Greedy Forward Stepwise Regression]
      Given your data $\mathcal{D}$, let's first standardize it to have mean $0$ and variance $1$.\footnote{This may or may not be a good idea, since the variance of each covariate can tell you a lot about the importance of the covariate.} With each covariate $X = (X_1, \ldots, X_n)$, we compute the correlation between it and the $Y$, which reduces to the inner product (since we standardized). 
      \begin{equation}
        \rho_j = \langle Y, X_j \rangle = \frac{1}{n} \sum_{i=1}^n Y_i X_{ij}
      \end{equation}
    \end{definition}
  ...
  \subsection{Lasso Regression}
    ...
    \end{equation}
    This gives us a convex problem, which we can then solve. In fact, it turns out that optimizing the risk given the L1 restriction on the norm is equivalent to minimizing the risk plus a L1 penalty. Therefore, there exists a pair $(L, \lambda)$ for which the two problems are equivalent 
    \begin{equation}
    ...
    Unfortunately, there is no closed form solution for this estimator, but in convex optimization, we can prove that this estimator is sparse. That is, for large enough $\lambda$, many of the components of $\hat{\beta}$ are $0$. The classical intuition for this is the figure below. 
